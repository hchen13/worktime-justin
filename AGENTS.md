...
7a. PM may inspect a feature branch or worktree for preliminary review, but Ethan-facing validation must use `stage` or an app/docs package built from `stage`. Do not ask Ethan to open the shared project `docs/index.html`, the app, or any normal validation path until the accepted work is actually visible there. If the shared project checkout cannot safely be switched to `stage`, PM must stop, say so, and route TL to provide a stage-built package or resolve the checkout state.
