...
6. PM owns `main` and the runnable integration branch `stage`; TL owns implementation branches that are neither `main` nor `stage`. Do not merge to `main` or `stage` outside the PM workflow.
7. `stage` is Ethan's runnable integration-acceptance branch, not QA's required test target. After PM accepts a runtime-impacting or docs-preview-impacting branch that should be shown to Ethan as the combined app/docs state, PM must promptly merge it into `stage`, resolve conflicts, build from `stage`, and record the `stage` commit/evidence on the card; if PM defers integration, the card must say why. QA may run focused target tests from a named branch, package, or independent worktree when the card says what is under test.
