...
7. `stage` is the shared validation branch for Ethan and QA. After PM accepts a runtime-impacting or docs-preview-impacting branch, PM must promptly merge it into `stage`, resolve conflicts, build from `stage`, and record the `stage` commit/evidence on the card; if PM defers integration, the card must say why.
