...
- `batch-03`: 补齐 O-W 主体词。`owl`、`octopus`、`oven` 已由 `WTJ-20260704-043` 补齐；剩余包括 `pear`、`pencil`、`pizza`、`queen`、`robot`、`rainbow`、`sun`、`tree`、`umbrella`、`van`、`violin` 等。
...
- `manifest.json` 共 100 个词；64 个 `ready`，36 个 `pending_sprite_stubbed`。
- 64 个 ready sprite 加 placeholder 均为 `1024x1024 RGBA`。
- 四角 alpha 均为 0。
