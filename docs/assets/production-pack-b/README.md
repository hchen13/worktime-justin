...
本包交付秘密词词池与 sprite 库 v1。目标词池一次性定为 100 个儿童常见英语词，其中第一批 37 个已有生产级透明 PNG，其余 63 个先接入统一 placeholder，并在 `missing-assets.json` 中列出后续批次，避免开发被素材缺口阻塞。

## 产物

- `manifest.json`: 100 词词池，字段包含 `word`、`first_letter`、`display_name`、`asset_path`、`sound_key`、`status`、`batch`、`intended_asset_path`。
- `sprites/`: 第一批 37 个生产级秘密词 sprite，均为 `1024x1024 RGBA` 透明 PNG。
- `stubs/secret-word-placeholder.png`: pending 词的临时占位素材，来自 A 包问号 token。
- `missing-assets.json`: 63 个待生产词的完整缺口清单和计划批次。
- `source/`: 三张生成源图及去背 alpha sheet。
- `contact-sheets/secret-word-sprites-v1-contact-sheet.png`: 暗底评审接触表。

## 第一批 ready 词

`airplane`, `ant`, `apple`, `ball`, `banana`, `basket`, `bell`, `cake`, `car`, `cat`, `cup`, `dog`, `door`, `drum`, `duck`, `egg`, `elephant`, `faucet`, `fish`, `flower`, `frog`, `grapes`, `hat`, `heart`, `horse`, `icecream`, `key`, `kite`, `lamp`, `leaf`, `moon`, `mouse`, `orange`, `pig`, `rocket`, `star`, `treasure`。

其中 `apple`、`ball`、`basket`、`car`、`cat`、`dog`、`star`、`treasure` 复用 v3 sprite 基准；`bell`、`door`、`faucet`、`horse`、`lamp` 复用 A 包任务道具；其余 24 个来自本卡三张新生成 sheet。

## 后续批次

- `batch-02`: 补齐 E-N 范围中缺口较多的常见物体，含 `alligator`、`eye`、`envelope`、`goat`、`gift`、`guitar`、`house`、`igloo`、`insect`、`island`、J/K/L/M/N 词等。
...
- `manifest.json` 共 100 个词；37 个 `ready`，63 个 `pending_sprite_stubbed`。
- 37 个 ready sprite 加 placeholder 均为 `1024x1024 RGBA`。
- 四角 alpha 均为 0。
- 可见像素中无 #ff00ff / 洋红残留。
