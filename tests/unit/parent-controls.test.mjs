...
function createSandbox(opts) {
  ...
  var shellMessages = [];
  // WTJ-20260707-004：parent-controls.js 新增了一个独立的 window 级 'keydown' 监听
  // （reset 口令识别，见该文件"reset 口令"一节），需要一个最小可用的 addEventListener 桩
  // ——真实存储按 type 分类的 handler 数组（同一 type 可能有多个监听器，虽然本文件目前只注册
  // 一个 keydown 监听），供下面 fireKeyDown() 手动派发。
  var listeners = {};
  var fakeWindow = {
  ...
  return {
    ...
    // key: 单字符（如 'r'）；opts 可传 { repeat: true } 等覆盖默认字段。
    fireKeyDown: function (key, extra) {
      ...
      (listeners.keydown || []).forEach(function (fn) { fn(evt); });
    }
  };
}
...
test('7c. 选中 auto：setMode 成功，语言提示区展示两种语言的可用性计数', function () {
  ...
  var note = findAll(sb.body, function (n) { return n.className === 'wtj-parent-settings-lang-note'; })[0];
  assert.ok(note.textContent.indexOf('72/72') !== -1, '应展示中文 72/72 完整的计数（WTJ-20260706-010 按键池扩容 40 条 ZH 交付后）');
  assert.ok(note.textContent.indexOf('53/72') !== -1, '应展示英文 53/72 不完整的计数（按键池 45 条 EN 已交付，drag/find EN 仍缺 19）');
...
// =====================================================================================
// 9. "reset" 口令（WTJ-20260707-004，验收标准 #2/#3）：任意界面（含锁屏）连续输入 "reset"
//    转发 { type: 'wtjResetPasscodeAttempt' } 给 shell。是否真的解锁（同日拒绝/跨日放行）
//    是 shell（main.swift 的 handleResetPasscodeAttempt()）的权威判定，不在本文件（纯 web
//    沙箱）覆盖范围——这里只验证"web 层正确识别输入并转发一次尝试"这条契约，与既有 8 节
//    "wtjReturnToParentMenu 由 shell 决定弹哪个菜单"是同一个覆盖边界取舍。
// =====================================================================================

function typeWord(sb, word) {
  for (var i = 0; i < word.length; i++) {
    sb.fireKeyDown(word[i]);
  }
}

test('9. 连续输入 "reset"（含大小写混合）触发一次 postMessage({ type: "wtjResetPasscodeAttempt" })', function () {
  var sb = createSandbox();
  typeWord(sb, 'ReSeT');
  assert.equal(sb.shellMessages.length, 1);
  assert.equal(JSON.stringify(sb.shellMessages[0]), JSON.stringify({ type: 'wtjResetPasscodeAttempt' }));
  console.log('PASS 9: 大小写混合输入 "ReSeT" 正确识别并转发一次 wtjResetPasscodeAttempt。');
});

test('9b. 锁屏状态下（wtjSetLockout(true, ...)）输入 reset 依然能识别并转发——不受 isInputSuspended() 影响', function () {
  var sb = createSandbox();
  sb.window.wtjSetLockout(true, 0);
  assert.equal(sb.PARENT_CONTROLS.isInputSuspended(), true, '前置条件：确认当前处于抑制状态（锁屏）');

  typeWord(sb, 'reset');

  assert.equal(sb.shellMessages.length, 1, '锁屏（isInputSuspended()=true）期间 reset 口令监听必须仍然生效，这是它与 keyboard.js/pointer.js 唯一刻意的不对称之处');
  assert.equal(JSON.stringify(sb.shellMessages[0]), JSON.stringify({ type: 'wtjResetPasscodeAttempt' }));
  console.log('PASS 9b: 锁屏期间输入 reset 仍正确转发，不受 isInputSuspended() 门禁影响。');
});

test('9c. 设置面板打开期间（isSettingsPanelOpen()=true）输入 reset 同样不受抑制影响', function () {
  var sb = createSandbox();
  sb.window.wtjShowSettingsPanel({ dailyLimitMinutes: 30, usedSecondsToday: 0, remainingSecondsToday: 1800, locked: false });
  assert.equal(sb.PARENT_CONTROLS.isInputSuspended(), true, '前置条件：设置面板打开也会令 isInputSuspended() 为 true');

  typeWord(sb, 'reset');

  assert.equal(sb.shellMessages.length, 1);
  console.log('PASS 9c: 设置面板打开期间输入 reset 仍正确转发。');
});

test('9d. 拼错的序列不触发（如 "reser"、无关字母）', function () {
  var sb = createSandbox();
  typeWord(sb, 'reser'); // 最后一个字母打错，不构成 "reset"
  assert.equal(sb.shellMessages.length, 0, '拼错的序列不应触发');

  typeWord(sb, 'abcde');
  assert.equal(sb.shellMessages.length, 0, '无关字母序列不应触发');
  console.log('PASS 9d: 拼错/无关字母序列均不触发 wtjResetPasscodeAttempt。');
});

test('9e. 命中后 buffer 清空：连续两次输入 "reset" 触发两次独立的 postMessage', function () {
  var sb = createSandbox();
  typeWord(sb, 'reset');
  typeWord(sb, 'reset');
  assert.equal(sb.shellMessages.length, 2, '命中后应清空 buffer，允许再次从头拼出并再次触发');
  assert.equal(JSON.stringify(sb.shellMessages[0]), JSON.stringify({ type: 'wtjResetPasscodeAttempt' }));
  assert.equal(JSON.stringify(sb.shellMessages[1]), JSON.stringify({ type: 'wtjResetPasscodeAttempt' }));
  console.log('PASS 9e: 命中后 buffer 清空，连续两次输入各自独立触发。');
});

test('9f. 数字/符号穿插不打断已积累的字母缓冲（如 "12" 夹在中间）', function () {
  var sb = createSandbox();
  sb.fireKeyDown('r');
  sb.fireKeyDown('e');
  sb.fireKeyDown('1'); // 数字：忽略，不打断
  sb.fireKeyDown('2'); // 数字：忽略，不打断
  sb.fireKeyDown('s');
  sb.fireKeyDown('e');
  sb.fireKeyDown('t');
  assert.equal(sb.shellMessages.length, 1, '数字穿插不应打断已积累的 "re"+"set" 拼接');
  console.log('PASS 9f: 数字穿插不打断已积累字母缓冲，仍能正确识别 reset。');
});

test('9g. e.repeat=true（长按自动连发）的按键被忽略，不计入缓冲', function () {
  var sb = createSandbox();
  sb.fireKeyDown('r');
  sb.fireKeyDown('r', { repeat: true }); // 长按重复：应被忽略，不应把 buffer 变成 "rr"
  sb.fireKeyDown('e');
  sb.fireKeyDown('s');
  sb.fireKeyDown('e');
  sb.fireKeyDown('t');
  assert.equal(sb.shellMessages.length, 1, 'repeat 事件被忽略后，"r"+"e"+"s"+"e"+"t" 仍应正确拼出 reset（若未忽略 repeat，buffer 会变成 "rrese" 之类而错过命中窗口）');
  console.log('PASS 9g: e.repeat=true 的按键被正确忽略，不干扰真实的 reset 拼词。');
});

test('9h. window.webkit 桥缺失时输入 reset 不抛错（仅静默失败，同 6d/8c 场景）', function () {
  var sb = createSandbox({ omitWebkitBridge: true });
  assert.doesNotThrow(function () {
    typeWord(sb, 'reset');
  });
  console.log('PASS 9h: webkit 桥缺失时输入 reset 不抛错。');
});
