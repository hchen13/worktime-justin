...
// opts.includePointer / includeKeyboard / includeHud / includeAudio / includeTask: false 时
// 不把对应 stub 挂到 window 上（模拟该依赖完全缺失）；stub 对象本身始终创建，测试代码可以
// 安全读取其调用记录（应保持为空数组）。
...
test('点击任务：onClick 触发 completeTask，切换 targetSpriteActive 贴图与 data-anim-state（真实 manifest 的 lamp-off/lamp-on 均别名到 task-props/lamp.png）；active 态在完成后仍可见约 800ms 才被移除（P1-3）', function () {
  var env = createSandbox();
  ...
  assert.ok(el.src.indexOf('assets/task-props/lamp.png') !== -1, '灯具初始素材应解析到 task-props/lamp.png');
  ...
  assert.ok(el.src.indexOf('assets/task-props/lamp.png') !== -1, 'targetSpriteActive（lamp-on stub）应仍解析到同一张 lamp.png（别名）');

  // P1-3：pointer target 立即 unregister，但 DOM 完成瞬间仍在——active 态（灯亮占位视觉）需要
  ...
  // 快进假时钟约 800ms：延迟移除的定时器到期，DOM 叠层才真正被摘除。
  env.clock.advance(800);
  assert.equal(getOverlayRoot(env).children.length, 0, '快进 ~800ms 后叠层应被移除');
});
