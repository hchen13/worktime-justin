...
test('P1-1：type 内 example 轮转不死锁——drag 的 apple-basket/dog-home、press 的 letter-a/digit-3 都应该轮到过（修复前 TASK_TYPES.length=4 与 examples.length=2 同奇偶，会让 drag 恒选 examples[0]、press 恒选 examples[1]，dog-home/press-letter-a 永不可达）', function () {
  ...
  // 覆盖 examples 下标 0~3（Math.floor(counter/4) 在 16 次内最大到 3），不足以覆盖全部 7 个——
  // 这不是死锁回归，只是本用例固定 16 次的覆盖范围有限；下方独立的"全覆盖"用例用动态迭代次数
...
// =============================================================================================
// 2c.（WTJ-20260705-004 Phase A）P1-1 扩展：四类任务型 example 轮转在真实 manifest 下全覆盖，
// 不因 examples.length 变化死锁——drag=2/click=3/find=12（pt2 新增）/press=7（pt3 新增）。
// 用真实 manifest 的 examples.length（而不是硬编码的具体 id 列表）动态推导需要多少次点击 +
// 动态推导期望覆盖到的完整 id 集合，这样以后任何一个类型的 examples 数组再次改变长度，
// 本用例都会自动跟着调整覆盖轮次，不需要每次手改断言数值——这正是"扩展 P1-1 覆盖所有
// examples.length 变化类型"的落地方式。
// =============================================================================================

test('P1-1（扩展）：四类任务型 example 轮转在真实 manifest 下全覆盖，覆盖 drag/click/find/press 各自完整的 examples 集合，不因 examples.length 变化死锁', function () {
  var env = createSandbox();
  var TASK_TYPES_LEN = 4; // 镜像 task-templates.js 源码 `TASK_TYPES = ['drag','click','find','press']`。
  var types = ['drag', 'click', 'find', 'press'];
  var seenByType = { drag: {}, click: {}, find: {}, press: {} };
  ...
  // example 下标 = Math.floor(questionClickCounter / TASK_TYPES_LEN) % examples.length；要让
  // 最长的那个 examples 数组（当前是 find 的 12 条）的每个下标都至少轮到一次，
  // Math.floor(counter/4) 需要取到 0..maxExamplesLen-1，即 counter 至少要到
...
test('拖拽任务（pt1）：distractorSprites 渲染为纯装饰元素——不注册 pointer target，不参与 draggable/dropTargetIds，emphasize 阶段不强调它们', function () {
  var env = createSandbox();
  var info = startTaskOfType(env, 'drag'); // 首次点击问号 counter=0 即为 drag，落到 examples[0]=apple-to-basket。
  assert.equal(info.taskId, 'drag-apple-to-basket');
...
test('072/025：click 类型五个 example（lamp/faucet/horse/door/bell）在轮转中都应该被生成到过（修复前 manifest.js click.examples 只有 lamp 一条，faucet/horse 恒不可达；WTJ-20260705-025 追加 door/doorbell 点击任务后扩到五条）', function () {
  ...
  // click-doorbell-ring 两条后从 3 条扩到 5 条）：一个完整周期是 4*5=20 次点击就足够让五个
  // example 下标都轮到至少一次（与 P1-1 测试同一推导：example 下标 =
  // Math.floor(questionClickCounter / TASK_TYPES.length) % examples.length）。24 次点击
  // 覆盖一个完整周期外加 4 次余量，足够验证全部 5 条。
  for (var i = 0; i < 24; i++) {
    env.taskStub.clickQuestion();
    var info = env.TT.getActiveTaskInfo();
    assert.ok(info, '每次点击问号都应该生成一个进行中任务');
  ...
  assert.deepEqual(
    ...
    ['click-door-open', 'click-doorbell-ring', 'click-faucet-on', 'click-horse-run', 'click-lamp-on'],
...
test('025：door 点击任务端到端——轮转可达 click-door-open，目标可见可点（assets/task-props/door.png），即使引擎存在也走静态 <img>（door 不在 PROP_ANIM_STATE_MAP 内），onClick 命中后 data-anim-state 切到 active 并 completeTask + 防御式播放 learningWord "door"', function () {
  var env = createSandbox();
  ...
  var el = targetCall.config.el;
  assert.equal(String(el.tagName).toLowerCase(), 'img', 'door 不在 PROP_ANIM_STATE_MAP 内（v1_boundary.deferred_to_v2），即使 WTJ_FRAME_ANIM 引擎存在也应回退静态 <img>，不是 <canvas>');
  assert.ok(el.src.indexOf('assets/task-props/door.png') !== -1, 'door 目标应解析到 assets/task-props/door.png（TASK_PROPS_FILENAMES 白名单，Pack A 已交付真实素材，非 stub）');
  assert.equal(el.getAttribute('data-anim-state'), 'idle', 'door 在 ANIM_STATE_FILENAMES 预留清单内，初始应为 idle（即使静态回退也保留这个属性接口）');
  ...
  assert.equal(el.getAttribute('data-anim-state'), 'active', '点击命中后应把 data-anim-state 切到 active（占位视觉接口，见 task-templates.css）');

  // learningWord: 'door' 命中 secretWords.pool 的 'door' 词条（D 组，已交付），完成后应防御式
  // 重播这个词——与 apple/lamp/dog 等既有 learningWord 落地方式完全一致。
...
test('025：doorbell 点击任务端到端——轮转可达 click-doorbell-ring，目标可见可点（assets/task-props/bell.png），静态 <img> 回退，onClick 命中后 completeTask + 防御式播放 learningWord "bell"', function () {
  var env = createSandbox();
  ...
  var el = targetCall.config.el;
  assert.equal(String(el.tagName).toLowerCase(), 'img', 'bell 不在 PROP_ANIM_STATE_MAP 内，应回退静态 <img>');
  assert.ok(el.src.indexOf('assets/task-props/bell.png') !== -1, 'bell 目标应解析到 assets/task-props/bell.png（Pack A 已交付真实素材，非 stub）');
...
test('025：door/doorbell 的 voicePrompt 在真实 manifest 里刻意为空字符串（024/084 尚未交付这两条中文语音）——task.js 的既有短路（!voicePrompt -> 不发起任何播放）已经保证这是"安静跳过"而不是"播放不匹配的语音"，本用例钉住这个数据形状不被后续误改成随手指向某个不存在/不相关的路径', function () {
  var env = createSandbox();
  ...
  assert.equal(doorExample.voicePrompt, '', 'click-door-open.voicePrompt 应为空字符串（no-silent-fallback 占位，待 024/084 交付）');
  assert.equal(bellExample.voicePrompt, '', 'click-doorbell-ring.voicePrompt 应为空字符串（同上）');
...
test('025：drag 池扩容——manifest.tasks.templates.drag.examples 恰好 8 条（原 2 条 + 新增 6 条），新增 6 条的 id/学习词/voicePrompt 数据形状符合 no-silent-fallback 约定（voicePrompt 为空字符串，待 024/084 交付；learningWord 均能在 secretWords.pool 命中）', function () {
  var env = createSandbox();
  ...
  newIds.forEach(function (id) {
    ...
    assert.equal(ex.voicePrompt, '', id + '.voicePrompt 应为空字符串（024/084 尚未交付，no-silent-fallback 占位）');
...
test('完成任务后防御式点亮 WTJ_HUD 状态灯（真实 manifest count=3，按顺序轮转 0,1,2,0）并尝试播放 successAudio', function () {
  ...
  // WTJ-20260705-004 Phase A（pt5）：drag-apple-to-basket/click-lamp-on/find-the-dog 都带
  // learningWord（'apple'/'lamp'/'dog'），完成后应各自触发一次 playWord()；press-letter-a
  // 未设置 learningWord（字母任务不是词汇学习），不应触发——因此 4 次完成只应有 3 次 playWord。
  ...
  var expectedWords = ['apple', 'lamp', 'dog'];
  var expectedAudioFiles = ['audio/words/apple.m4a', 'audio/words/lamp.m4a', 'audio/words/dog.m4a'];
