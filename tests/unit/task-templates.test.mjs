...
// 任务开始时的双语词语播放断言（见 playFindWordBilingualDefensive() 消费方式）。
...
test('寻找任务：onHover 命中触发 completeTask；干扰项渲染但不注册 pointer target', function () {
  ...
  // randomPool.sampleSize=2），首个 find 抽取（createSandbox() 默认 identity RNG 下恒为
  // secretWords.pool[0]='apple'）仍然是 1 个目标 + 2 个干扰项，元素数量口径不变。
...
// =============================================================================================
// 12. WTJ-20260706-012（EN-side 随机 word-card find driver）：find 类型的 target/distractor
// 改从 secretWords.pool（现场核对 100 词，xylophone 已在 011 卡删除，不是 101）任意抽取，取代
// 12 条精选 example 作为主路径（那 12 条继续作为 manifest 缺失 randomPool 配置/pool 为空时的
// 回退路径，见 task-templates.js drawWordCardFind() 说明）。本节只覆盖 EN 半部分——ZH 半部分
// （wordAudioFileZh/中文语音）门禁在 011/008，不在本卡范围。
// =============================================================================================

test('WTJ-20260706-012：find 随机 word-card 抽取——每次抽取的 target 与 distractor 均来自 secretWords.pool、互不重复，distractor 数量恒为 randomPool.sampleSize', function () {
  ...
  var sampleSize = env.manifest.tasks.templates.find.randomPool.sampleSize;
  ...
  for (var round = 0; round < 30; round++) {
    ...
    assert.equal(spriteFiles.length, 1 + sampleSize, '第 ' + round + ' 轮：应渲染 1 个 target + ' + sampleSize + ' 个 distractor');
