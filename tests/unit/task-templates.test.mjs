...
// WTJ-20260707-009：按键任务屏幕键盘提示挂在独立于 ensureOverlayRoot()（.wtj-tt-root）的一层
// .wtj-key-hint-root 上，直接 appendChild 到 document.body——不能像 getOverlayRoot() 那样假设
// 固定下标（.wtj-tt-root 是否已经被前面的 drag/click/find 轮转懒创建、算作 body.children[0]，
// 取决于测试驱动到 press 之前经过了哪些类型），这里按 className 子串遍历 body 的直接子节点找。
function getKeyHintRoot(env) {
  var children = env.docStub.body.children;
  for (var i = 0; i < children.length; i++) {
    if (children[i] && typeof children[i].className === 'string' && children[i].className.indexOf('wtj-key-hint-root') !== -1) {
      return children[i];
    }
  }
  return null;
...
// 纯外层（主 realm）递归遍历——不依赖 querySelector（本文件手写的沙箱 document stub 不提供这个
// 方法，task-templates.js 生产代码本身也刻意不依赖它，见该文件 createKeyHintKeyEl() 注释）；
// 这里只是测试代码自己在已经拿到的 plain object 树上按 className 子串做深度优先查找，与
// production 代码是否使用 querySelector 无关。
function findAllByClassSubstr(root, cls, acc) {
  acc = acc || [];
  if (!root) {
    return acc;
  }
  if (typeof root.className === 'string' && root.className.indexOf(cls) !== -1) {
    acc.push(root);
  }
  var children = root.children || [];
  for (var i = 0; i < children.length; i++) {
    findAllByClassSubstr(children[i], cls, acc);
  }
  return acc;
}

function findOneByClassSubstr(root, cls) {
  var all = findAllByClassSubstr(root, cls);
...
// =============================================================================================
// 6. 按键任务（REQ-TASK-10）：判定仍是纯键盘匹配，不注册 pointer target；WTJ-20260707-009 起
//    额外渲染一套独立于 .wtj-tt-root 的屏幕键盘提示 DOM（.wtj-key-hint-root，见下方「6b」小节）。
// =============================================================================================

test('按键任务：不注册任何 pointer target（不占用 .wtj-tt-root，只渲染独立的 .wtj-key-hint-root）；WTJ_KEYBOARD.onLetter 命中 targetKey 才 completeTask，不命中不完成', function () {
  var env = createSandbox();
  ...
  // 创建就不会被移除，见 getOverlayRoot() 注释）——WTJ-20260707-009 起按键任务会额外渲染屏幕
  // 键盘提示 DOM，但那是独立的 .wtj-key-hint-root（直接挂 document.body，见 getKeyHintRoot()），
  // 不会占用 .wtj-tt-root 这个共享叠层容器，本次 press 任务也不应再新增任何 pointer 注册。
  var root = getOverlayRoot(env);
  assert.ok(root, '此前 drag 任务已经触发过 overlay root 的懒创建');
  assert.equal(root.children.length, 0, '按键任务不应往共享 .wtj-tt-root 里渲染任何 DOM（root 下应无任何遗留/新增子节点）');
  ...
  assert.equal(root.children.length, 0, '按键任务完成后共享 overlay root 下仍不应有任何子节点');
});

// =============================================================================================
// 6b. WTJ-20260707-009：press 问号任务屏幕键盘提示（消费 WTJ-20260707-005 设计规格）
// =============================================================================================

test('屏幕键盘提示：默认界面与非 press 任务（drag/click/find）不应渲染 .wtj-key-hint-root', function () {
  var env = createSandbox();
  assert.equal(getKeyHintRoot(env), null, '默认空闲界面不应有任何键盘提示 DOM');

  ['drag', 'click', 'find'].forEach(function (type) {
    startTaskOfType(env, type);
    assert.equal(getKeyHintRoot(env), null, type + ' 任务激活时不应渲染 .wtj-key-hint-root');
    env.taskStub.dismissActive();
  });
});

test('屏幕键盘提示：press 任务激活时渲染 .wtj-key-hint-root，字母 A 高亮为 wtj-kh-key--target，气泡只显示 "A"', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('A') });
  startTaskOfType(env, 'press');

  var root = getKeyHintRoot(env);
  assert.ok(root, 'press 任务激活时应渲染 .wtj-key-hint-root');

  var targetKeys = findAllByClassSubstr(root, 'wtj-kh-key--target');
  assert.equal(targetKeys.length, 1, '整张键盘应恰好一颗键被标记为 wtj-kh-key--target');
  assert.equal(targetKeys[0].textContent, 'A', '高亮键的键帽文字应是目标字母本身');

  var bubbleTarget = findOneByClassSubstr(root, 'wtj-kh-bubble-target');
  assert.ok(bubbleTarget, '气泡应渲染 .wtj-kh-bubble-target');
  assert.equal(bubbleTarget.textContent, 'A', '气泡只应显示目标键本身，不出现"目标键"之类的解释文字');
  assert.equal(findOneByClassSubstr(root, 'wtj-kh-bubble-combo'), null, '字母任务的气泡不应出现数字专属的组合结构');
});

test('屏幕键盘提示：数字 3 的气泡显示主字符 + 上档符号组合（"3" + "#"），不只显示单一字符', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('3') });
  startTaskOfType(env, 'press');

  var root = getKeyHintRoot(env);
  var combo = findOneByClassSubstr(root, 'wtj-kh-bubble-combo');
  assert.ok(combo, '数字任务的气泡应该是 main+shift 组合结构');
  var mainEl = findOneByClassSubstr(combo, 'wtj-kh-bubble-main');
  var shiftEl = findOneByClassSubstr(combo, 'wtj-kh-bubble-shift');
  assert.equal(mainEl.textContent, '3');
  assert.equal(shiftEl.textContent, '#');

  var targetKeys = findAllByClassSubstr(root, 'wtj-kh-key--target');
  assert.equal(targetKeys.length, 1, '数字键也应该走普通暖黄高亮（wtj-kh-key--target），不是 Space/Enter/方向键专用的青色');
});

test('屏幕键盘提示：符号 "+" 不是独立键，命中的是数字行 "=" 键，气泡只显示 "+" 本身（不显示 "="）', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('+') });
  startTaskOfType(env, 'press');

  var root = getKeyHintRoot(env);
  var targetKeys = findAllByClassSubstr(root, 'wtj-kh-key--target');
  assert.equal(targetKeys.length, 1, '"+" 应该命中且只命中一颗物理键');
  // 命中键是 "="/"+" 复合键：键帽同时渲染 main="=" 与 shift="+" 两个 span（005 spec「布局」
  // 一节："'+' 不是独立键，必须作为 '=' 键的上档符号出现"）。
  var mainSpan = findOneByClassSubstr(targetKeys[0], 'wtj-kh-key-main');
  var shiftSpan = findOneByClassSubstr(targetKeys[0], 'wtj-kh-key-shift');
  assert.equal(mainSpan.textContent, '=', '"+" 命中的物理键键帽主字符应是 "="');
  assert.equal(shiftSpan.textContent, '+', '"+" 命中的物理键键帽上档符号应是 "+"');

  var bubbleTarget = findOneByClassSubstr(root, 'wtj-kh-bubble-target');
  assert.equal(bubbleTarget.textContent, '+', '气泡只应显示要按的 "+" 本身，不应该显示 "=" 或组合');
});

test('屏幕键盘提示：Space 用青色 soft-target 高亮，气泡显示 "␣"', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('Space') });
  startTaskOfType(env, 'press');

  var root = getKeyHintRoot(env);
  var softTargets = findAllByClassSubstr(root, 'wtj-kh-key--soft-target');
  assert.equal(softTargets.length, 1, 'Space 应该命中且只命中一颗键，走特殊高亮（青色）');
  assert.equal(findAllByClassSubstr(root, 'wtj-kh-key--target').length, 0, 'Space 不应触发普通暖黄高亮');

  var bubbleTarget = findOneByClassSubstr(root, 'wtj-kh-bubble-target');
  assert.equal(bubbleTarget.textContent, '␣', 'Space 气泡应显示 ␣ 符号本身');

  // 全局唯一一颗 space（005 spec「布局」一节："space 只能有一颗"）。
  var spaceKeys = findAllByClassSubstr(root, 'wtj-kh-key--space');
  assert.equal(spaceKeys.length, 1, '整张键盘应该只有一颗 space 宽度的键');
});

test('屏幕键盘提示：Enter 用青色 soft-target 高亮，键帽文字仍是 "return"，气泡收窄成 "↵"（不显示 "Enter"/"return" 文字）', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('Enter') });
  startTaskOfType(env, 'press');

  var root = getKeyHintRoot(env);
  var softTargets = findAllByClassSubstr(root, 'wtj-kh-key--soft-target');
  assert.equal(softTargets.length, 1);
  assert.equal(softTargets[0].textContent, 'return', '005 预览截图里 return 键帽命中时仍保留 "return" 文字，只是变色，不是本卡改造对象');

  var bubbleTarget = findOneByClassSubstr(root, 'wtj-kh-bubble-target');
  assert.equal(bubbleTarget.textContent, '↵', '气泡应收窄成纯符号 ↵，不出现 "Enter"/"return" 这类文字');
});

test('屏幕键盘提示：方向键 ArrowUp 用青色 soft-target 高亮，气泡显示 "↑"', function () {
  ...
  var softTargets = findAllByClassSubstr(root, 'wtj-kh-key--soft-target');
  assert.equal(softTargets.length, 1);
  assert.equal(softTargets[0].textContent, '↑');

  var bubbleTarget = findOneByClassSubstr(root, 'wtj-kh-bubble-target');
  assert.equal(bubbleTarget.textContent, '↑');
});

test('屏幕键盘提示：命中成功时高亮键补一次 wtj-kh-key--success 成功闪光 class（220ms 闪光，随后交给既有完成态可见窗口）', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('A') });
  startTaskOfType(env, 'press');
  var root = getKeyHintRoot(env);
  var targetKeyEl = findOneByClassSubstr(root, 'wtj-kh-key--target');
  assert.ok(targetKeyEl);
  assert.equal(targetKeyEl.classList._list.indexOf('wtj-kh-key--success'), -1, '命中前不应该有成功闪光 class');

  env.keyboardStub.pressLetter('A');

  assert.equal(env.taskStub.completeTaskCalls.length, 1, '命中应该正常完成任务（成功闪光不影响既有判定路径）');
  assert.ok(targetKeyEl.classList._list.indexOf('wtj-kh-key--success') !== -1, '命中后应该给高亮键加上 wtj-kh-key--success 成功闪光 class');
});

test('屏幕键盘提示：完成态延迟移除——命中瞬间 DOM 仍在（可见窗口内），~800ms 后 .wtj-key-hint-root 才被移除', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('A') });
  startTaskOfType(env, 'press');
  assert.ok(getKeyHintRoot(env), '任务激活时应存在键盘提示 DOM');

  env.keyboardStub.pressLetter('A');
  assert.ok(getKeyHintRoot(env), '命中瞬间 DOM 应仍然存在（延迟移除，P1-3 完成态可见窗口尚未到期）');

  env.clock.advance(799);
  assert.ok(getKeyHintRoot(env), '799ms 时仍不应被移除（hold 是 800ms 下限）');

  env.clock.advance(1);
  assert.equal(getKeyHintRoot(env), null, '恰好 800ms 后 .wtj-key-hint-root 应该被移除');
});

test('屏幕键盘提示：onDismiss（超时/键盘转移）应立即清理 .wtj-key-hint-root，不像完成路径那样有可见窗口', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('A') });
  startTaskOfType(env, 'press');
  assert.ok(getKeyHintRoot(env));

  env.taskStub.dismissActive();

  assert.equal(getKeyHintRoot(env), null, 'dismiss 应该立即（不延迟）清理键盘提示 DOM');
});

test('屏幕键盘提示：15s hint / 30s emphasize phase 把 wtj-tt-hint / wtj-tt-emphasize 加到 .wtj-key-hint-root 本身（复用既有 handleTaskPhase() 通用逻辑，靠 CSS 后代选择器只强调高亮键）', function () {
  var env = createSandbox({ manifestOverrideSrc: makeSinglePressManifestSrc('A') });
  startTaskOfType(env, 'press');
  var root = getKeyHintRoot(env);

  env.taskStub.firePhase({ phase: 'hint' });
  assert.ok(root.classList._list.indexOf('wtj-tt-hint') !== -1, 'hint 阶段应该把 wtj-tt-hint 加到键盘提示根节点');

  env.taskStub.firePhase({ phase: 'emphasize' });
  assert.ok(root.classList._list.indexOf('wtj-tt-emphasize') !== -1, 'emphasize 阶段应该把 wtj-tt-emphasize 加到键盘提示根节点');
});
...
// 文件唯一需要为 013 新增的回归护栏。
// ============================================================================================
// WTJ-20260707-009：css 文件里现在有两个 @media (prefers-reduced-motion: reduce) 块（013 的
// .wtj-tt-prop 一个 + 本卡键盘提示新增一个）——旧版单一贪婪正则 /@media(...)\{([\s\S]*)\}\s*$/
// 假设"整个文件只有一个、且在文件末尾"的兜底块，两个块共存时它会把中间所有内容（含紧凑桌面
// 断点等无关 CSS）都吞进 blockBody，虽然凑巧不影响旧断言结果，但语义已经不对，也没有真正
// 覆盖新块自己的门禁。改用括号计数逐个提取每一个 @media (prefers-reduced-motion: reduce) 块
// 各自的独立 body，不依赖"只有一个/在文件末尾"这两个不再成立的假设。
function extractReducedMotionBlocks(css) {
  var marker = '@media (prefers-reduced-motion: reduce)';
  var blocks = [];
  var searchFrom = 0;
  while (true) {
    var markerIdx = css.indexOf(marker, searchFrom);
    if (markerIdx === -1) {
      break;
    }
    var openIdx = css.indexOf('{', markerIdx);
    if (openIdx === -1) {
      break;
    }
    var depth = 1;
...
test('task-templates.css 静态契约：prefers-reduced-motion 兜底块内 .wtj-tt-prop / .wtj-key-hint-root 相关规则均应加 html:not([data-wtj-motion-forced]) 门', function () {
  var css = readFileSync(TASK_TEMPLATES_CSS_PATH, 'utf8');
  var blocks = extractReducedMotionBlocks(css);
  assert.equal(blocks.length, 2, 'task-templates.css 应恰好有两个 prefers-reduced-motion 兜底块：013 的 .wtj-tt-prop 一个、WTJ-20260707-009 键盘提示新增一个');
  var wholeBody = blocks.join('\n');

  [
    '.wtj-tt-prop',
    '.wtj-tt-prop.wtj-tt-emphasize',
    '.wtj-key-hint-root',
    '.wtj-key-hint-root .wtj-kh-bubble',
    '.wtj-key-hint-root .wtj-kh-key'
  ].forEach(function (selector) {
    var gatedSelector = 'html:not([data-wtj-motion-forced]) ' + selector;
    assert.ok(
      wholeBody.indexOf(gatedSelector) !== -1,
      'reduced-motion 兜底块应给 ' + selector + ' 加 html:not([data-wtj-motion-forced]) 门，实际未找到 "' + gatedSelector + '"'
    );
  });

  ['.wtj-tt-prop', '.wtj-key-hint-root', '.wtj-kh-key', '.wtj-kh-bubble'].forEach(function (bareClass) {
    var re = new RegExp('(^|[,{};]\\s*)\\' + bareClass + '\\b');
    blocks.forEach(function (block, idx) {
      var bareMatch = block.match(re);
      assert.equal(bareMatch, null, '第 ' + (idx + 1) + ' 个兜底块内不应该存在未加 html:not([data-wtj-motion-forced]) 门的裸 ' + bareClass + ' 选择器，命中：' + (bareMatch && bareMatch[0]));
    });
  });
});
