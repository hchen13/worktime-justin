...
test('P1-1（扩展）：四类任务型 example 轮转在真实 manifest 下全覆盖，覆盖 drag/click/find/press 各自完整的 examples 集合，不因 examples.length 变化死锁', function () {
  ...
  // examples 数组（当前是 find 的 12 条）的每个下标都至少轮到一次，需要点击次数至少推进到
...
test('洗牌袋契约②：example 洗牌袋——真实洗牌下 drag/click/find/press 各自完整覆盖 manifest 里全部 example id，显式验证 drag-dog-home、press-letter-a 均可达（P1-1 回归钉子：修复前的旧版计数器公式会让这两个 example 永不可达，见文件头「历史记录」一节；现在结构性保证可达，不依赖任何数论巧合）', function () {
  ...
  // floor((N-4)/4)+1；而 examples.length 最大的 find（12 条）需要该类型至少连续出现
  // 2*12-1=23 次才能结构性保证覆盖一整袋，换算总点击数只需 N>=92；200 次留了一倍以上余量，
  // 其余三个类型（drag=8/click=5/press=7）需要的总点击数下限更低。
  var N = 200;
  for (var i = 0; i < N; i++) {
    env.taskStub.clickQuestion();
    var info = env.TT.getActiveTaskInfo();
  ...
  ['drag', 'click', 'find', 'press'].forEach(function (t) {
