...
test('3. missing-audio.json 全部 53 条 taskVoice.voicePromptPath 逐一 playTaskVoice -> 路径正确 + 真实文件在 + silent:false', function () {
  ...
  assert.ok(Array.isArray(taskVoiceEntries) && taskVoiceEntries.length === 53, 'missing-audio.json taskVoice 段落应恰好有 53 条（原 8 + WTJ-20260706-010 按键池扩容 45 条 EN 整句）');
  ...
  return chain.then(function () {
    assert.equal(checkedCount, 53, '53 条 taskVoice 应全部逐一断言通过');
    console.log('PASS 3: missing-audio.json 全部 53 条 taskVoice.voicePromptPath 均 playTaskVoice 成功、路径未 mangle、silent:false。');
  });
});
...
test('7. Phase B 全量：manifest.js 四类模板已交付的 32 条 example 的 voicePrompt 均接线到交付的 .zh.m4a（与 tts-text-manifest.zh.json out 字段一致），文件存在非空，playTaskVoice 均 silent:false；WTJ-20260706-010 按键池扩容新增的 40 条 example 待 ASR-gated 生成交付，voicePrompt 暂留空（no-silent-fallback）', function () {
  ...
  assert.equal(deliveredExamples.length, 72, 'WTJ-20260706-010 按键池扩容 40 条 ZH 整句交付并接线后，四类模板已交付 example = 32 + 40 = 72');
  // WTJ-20260706-010：按键池扩容新增的 40 条 press example 的中文整句已由 ASR-gated CosyVoice3
  // 全部交付并接线（voicePrompt 从 '' 改指向各自 audio/tasks/press-<key>.zh.m4a），pending 回落
  // 到 0（与 025→024 那次"先扩池、voicePrompt 留空，音频到位后再接线"的既有先例同一模式，此刻已
  // 完成接线一步）。
  assert.equal(pendingExamples.length, 0, 'WTJ-20260706-010 按键池 40 条 ZH 整句全部交付接线后，不再有 voicePrompt 留空的待交付 example');
  ...
  return chain.then(function () {
    assert.equal(checkedCount, 72, '四类模板已交付的 72 条 example 都应逐一断言通过');
    console.log('PASS 7: manifest.js 全部 72 条 task example 的 voicePrompt 均接线到交付的 .zh.m4a，与 zh.json out 字段一致，文件存在非空，playTaskVoice 全部 silent:false（010 按键池 40 条 ZH 整句也已接线）。');
  });
});
