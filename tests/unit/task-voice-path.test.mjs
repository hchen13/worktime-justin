...
test('7. Phase B 全量：manifest.js 四类模板全部 32 条 example 中，24 条已交付的 voicePrompt 均接线到 084 交付的 .zh.m4a（与 tts-text-manifest.zh.json out 字段一致），文件存在非空，playTaskVoice 均 silent:false；WTJ-20260705-025 新增的 8 条待交付 example 的 voicePrompt 均确认留空（no-silent-fallback），且已在 tasksPending 登记文案草稿', function () {
  ...
  var zhTasksPending = zhManifest.tasksPending || {};
  ...
  assert.equal(deliveredExamples.length, 24, 'Phase B 已交付覆盖范围恒为 24 条 example（drag=2/click=3/find=12/press=7）');
  assert.equal(pendingExamples.length, 8, 'WTJ-20260705-025 新增的待交付 example 恒为 8 条（drag 池扩容 6 条 + door/doorbell 点击任务 2 条）');

  pendingExamples.forEach(function (ex) {
    assert.equal(ex.voicePrompt, '', ex.id + ': 待交付 example 的 voicePrompt 应确认为空字符串（no-silent-fallback，不静默指向不相关/不存在的文件）');
    var pendingEntry = zhTasksPending[ex.id];
    assert.ok(pendingEntry, ex.id + ' 应在 tts-text-manifest.zh.json 的 tasksPending 段落有拟定文案草稿，供 024/084 排期生成');
    assert.match(pendingEntry.out, /\.zh\.m4a$/, ex.id + ' 的 tasksPending 拟定 out 路径应遵循既有 .zh.m4a 命名约定');
  });
  ...
  return chain.then(function () {
    assert.equal(checkedCount, 24, '四类模板已交付的 24 条 example 都应逐一断言通过');
    console.log('PASS 7: manifest.js 已交付的 24 条 task example 的 voicePrompt 均接线到 084 交付的 .zh.m4a，与 zh.json out 字段一致，文件存在非空，playTaskVoice 全部 silent:false；新增的 8 条待交付 example 均确认 voicePrompt 留空且已登记文案草稿。');
  });
});
