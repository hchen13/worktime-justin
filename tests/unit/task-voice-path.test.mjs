...
//    文件名 stem 本就不同，是 078 报告里点名的两个刁钻样本之一）。断言：
...
test('2. 复刻 task.js 真实调用形态：真实 startTask(press-letter-a) 通过真实 audio.js 请求到未 mangle 的路径，且 playTaskVoice(该 voicePrompt) resolve silent:false', function () {
  ...
  assert.equal(taskDef.voicePrompt, 'audio/tasks/press-a.m4a');
  assert.notEqual(taskDef.id, 'press-a', 'id 与 voicePrompt 文件名 stem 本就不一致——正是 078 根因描述点名的刁钻样本');
  ...
  assert.equal(fetchCalls[0], 'audio/tasks/press-a.m4a', 'task.js 触发的真实 fetch 请求路径必须是未被 mangle 的真实路径');
  assert.notEqual(fetchCalls[0], 'audio/tasks/audiotaskspress-am4a.m4a');
  ...
  return sandbox.window.WTJ_AUDIO.playTaskVoice(taskDef.voicePrompt).then(function (r) {
    assert.equal(r.path, 'audio/tasks/press-a.m4a');
    assert.equal(r.silent, false, '复刻 task.js 的真实调用形态应 resolve silent:false');
    assert.equal(r.ok, true);
    console.log('PASS 2: 真实 task.js startTask(press-letter-a) -> 真实 audio.js fetch("audio/tasks/press-a.m4a")，未 mangle；playTaskVoice(voicePrompt) resolve silent:false。');
  });
});
