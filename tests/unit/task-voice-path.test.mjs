...
test('7. Phase B 全量：manifest.js 四类模板全部 32 条 example 的 voicePrompt 均接线到交付的 .zh.m4a（与 tts-text-manifest.zh.json out 字段一致），文件存在非空，playTaskVoice 均 silent:false（WTJ-20260705-024 CosyVoice3 全量重生成后，原 025 待交付的 8 条也已交付并接线，无一留空）', function () {
  ...
  assert.equal(deliveredExamples.length, 32, 'WTJ-20260705-024 全量重生成后已交付覆盖范围为 32 条 example（原 24 + 025 新增 8）');
  assert.equal(pendingExamples.length, 0, 'WTJ-20260705-024 交付后不再有 voicePrompt 留空的待交付 example');
