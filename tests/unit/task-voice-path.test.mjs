...
//    真实 manifest.js 的 drag/click/find/press 四类模板全部 24 条 example，逐一断言：
...
test('7. Phase B 全量：manifest.js 四类模板 24 条 example 的 voicePrompt 均已接线到 084 交付的 .zh.m4a（与 tts-text-manifest.zh.json out 字段一致），文件存在非空，playTaskVoice 均 silent:false', function () {
  ...
  assert.equal(allExamples.length, 24, 'Phase B 覆盖范围恒为 24 条 example（drag=2/click=3/find=12/press=7）');
  ...
  return chain.then(function () {
    assert.equal(checkedCount, 24, '四类模板全部 24 条 example 都应逐一断言通过');
    console.log('PASS 7: manifest.js 全部 24 条 task example 的 voicePrompt 均已接线到 084 交付的 .zh.m4a，与 zh.json out 字段一致，文件存在非空，playTaskVoice 全部 silent:false。');
  });
});
