...
test('1. pool 是合法数组，长度为 101（Pack B 100 词目标 + 1 条遗留 treasurechest）', function () {
  var manifest = loadManifest();
  var pool = manifest.secretWords.pool;
  assert.ok(Array.isArray(pool), 'secretWords.pool 必须是数组');
  assert.equal(pool.length, 101, 'pool 长度应为 101（Pack B 100 + 遗留 treasurechest 1 条）');
  assert.equal(manifest.secretWords.poolTargetSize, 100, 'poolTargetSize 应仍标注 Pack B 的 100 词目标');
  console.log('PASS 1: pool 是数组，长度 101，poolTargetSize=100。');
});
...
test('2. 每条词池条目字段合法：word 为纯小写字母、spriteFile/audioFile 均为非空字符串、audioFile 遵循 audio/words/<word>.m4a 约定', function () {
  ...
  console.log('PASS 2: 全部 101 条词池条目 word/spriteFile/audioFile 字段格式合法。');
});
...
test('3. 词池内无重复词（normalize 前字面值即唯一，避免同词重复条目导致匹配歧义）', function () {
  ...
  console.log('PASS 3: 101 条词池条目无重复 word。');
});
...
test('4. ready 词（非 spriteStub）的 spriteFile 经路径解析后，对应文件在 app/web/assets/ 下真实存在', function () {
  ...
  assert.equal(readyCount, 101, 'ready 词数量应精确为 101（Pack B 100 全 ready + 遗留 treasurechest 1），实际 ' + readyCount);
...
test('12. 命中冒烟：固定样本 Pack B 新增词（覆盖不同 batch，narrow refresh 后全部 ready），逐词单独喂入均能命中且 spriteUrl 指向真实专属 sprite', function () {
  ...
  // + 曾是 stub、现已转 ready 的 xylophone（X 组，卡 052）/ yoyo（Y 组，batch-04）
  // + narrow refresh 补齐的最后 Z 组 zebra（卡 054，现为 ready，下方单独断言其指向专属 sprite）。
  var samples = ['alligator', 'umbrella', 'wagon', 'xylophone', 'yoyo', 'zebra'];
  ...
  console.log('PASS 12: 固定样本词（alligator/umbrella/wagon/xylophone/yoyo/zebra）均正确命中并解析到各自专属 sprite；zebra → assets/sprites/zebra.png（已转 ready）。');
});
