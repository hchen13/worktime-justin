...
test('1. pool 是合法数组，长度为 100（Pack B 99 词 + 1 条遗留 treasurechest，xylophone 已删除，WTJ-20260706-011）', function () {
  var manifest = loadManifest();
  var pool = manifest.secretWords.pool;
  assert.ok(Array.isArray(pool), 'secretWords.pool 必须是数组');
  assert.equal(pool.length, 100, 'pool 长度应为 100（Pack B 99 + 遗留 treasurechest 1 条，xylophone 已删除后由 101 降至 100）');
  ...
  console.log('PASS 1: pool 是数组，长度 100（xylophone 已删除），poolTargetSize=100，pool 不含 xylophone。');
});
...
test('4. ready 词（非 spriteStub）的 spriteFile 经路径解析后，对应文件在 app/web/assets/ 下真实存在', function () {
  ...
  // narrow refresh 后 Pack B 已 100% ready（卡 052/054 补齐 X/Y/Z 组），词池 101 条全部 ready
  // （Pack B 100 ready + 遗留 treasurechest 1），无任何 stub。这个精确耦合是刻意的：将来若词池
  // 结构变化（增删词），编辑者需同步改此断言，用来提醒"改 pool 必须同步改断言"。
  // WTJ-20260706-011：删除 xylophone（教错 X 发音）后，Pack B ready 降至 99，总数降至 100。
  assert.equal(readyCount, 100, 'ready 词数量应精确为 100（Pack B 99 全 ready + 遗留 treasurechest 1，xylophone 已删除），实际 ' + readyCount);
  console.log('PASS 4: ' + readyCount + ' 条 ready 词的 sprite 文件均在 app/web/assets/ 下真实存在。');
});

test('5. 词池内已无 stub 词（Pack B 100% ready 后 spriteStub 已清零）；且无任何词条仍指向共享占位图', function () {
  var manifest = loadManifest();
  var pool = manifest.secretWords.pool;
  // narrow refresh 后 Pack B 全部产出完毕（卡 054 补齐最后 Z 组），词池内不应再有任何 spriteStub。
  ...
  console.log('PASS 5: 词池内 0 个 stub 词、0 个词条指向共享占位图 —— Pack B 100% ready 已落地。');
});
...
test('10. app/web/assets/sprites/ 目录 PNG 与 pool 引用文件名双向一致（既无缺失、也无未被引用的孤儿）', function () {
  ...
  // 已知白名单孤儿：Pack B 100% ready 后，共享占位图 secret-word-placeholder.png 不再被任何 pool
...
test('12. 命中冒烟：固定样本 Pack B 新增词（覆盖不同 batch，narrow refresh 后全部 ready），逐词单独喂入均能命中且 spriteUrl 指向真实专属 sprite', function () {
  ...
  // + 曾是 stub、现已转 ready 的 xray（X 组，卡 052；X 组另一词 xylophone 已于 WTJ-20260706-011
  // 删除，因其教错 X 发音，不再作为样本）/ yoyo（Y 组，batch-04）
  // + narrow refresh 补齐的最后 Z 组 zebra（卡 054，现为 ready，下方单独断言其指向专属 sprite）。
  var samples = ['alligator', 'umbrella', 'wagon', 'xray', 'yoyo', 'zebra'];
  ...
  console.log('PASS 12: 固定样本词（alligator/umbrella/wagon/xray/yoyo/zebra）均正确命中并解析到各自专属 sprite；zebra → assets/sprites/zebra.png（已转 ready）。');
});
