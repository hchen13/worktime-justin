...
test('3. 47 条 example 全部已交付：voicePrompt 均非空（无一 pending），且 M 沿用 008 已批准的干净 press-m.zh.m4a（本卡未重新生成/替换）', function () {
  var manifest = loadManifest();
  var examples = manifest.tasks.templates.press.examples;
  var delivered = examples.filter(function (e) { return !!e.voicePrompt; });
  var pending = examples.filter(function (e) { return !e.voicePrompt; });

  // WTJ-20260706-010：40 条按键池扩容目标的中文整句已由 ASR-gated CosyVoice3 全部交付并接线，
  // 加上本卡之前就已交付的 7 条（press-a/b/m/s/3/5/7），press.examples 现在 47 条全部有 voicePrompt。
  assert.equal(delivered.length, 47, '全部 47 条 press example 都应已接线 voicePrompt（40 条 010 新增 + 7 条历史）');
  assert.equal(pending.length, 0, '不应再有 voicePrompt 为空的 pending example（40 条 ZH 整句均已交付接线）');

  // 卡片验收标准 #3 回归钉子：M 仍必须指向 008 修复过的干净 press-m.zh.m4a，绝不能被本轮的任何
  // 批量生成覆盖（008 专门修复了「按下字母M」误读，press-m 是明令不得重生成的已批准文件）。
  var m = examples.find(function (e) { return e.targetKey === 'M'; });
  ...
  // 历史已交付的 7 条仍指向其原有 stem（没有被扩容误改指向别处）。
  PRE_EXISTING_DELIVERED_KEYS.forEach(function (key) {
    var ex = examples.find(function (e) { return e.targetKey === key; });
    assert.ok(ex && ex.voicePrompt.indexOf('.zh.m4a') !== -1, 'targetKey ' + key + ' 应仍指向其 ZH 整句 voicePrompt');
  });
  console.log('PASS 3: 47 条全部已交付、0 pending；M 沿用 008 批准版本，7 条历史交付未被误改。');
});

test('4. 全部 47 条已交付 example：voicePrompt(.zh.m4a) 磁盘文件真实存在且非空、与 tts-text-manifest.zh.json 的 out 字段一致（不是凭 id 猜文件名）', function () {
  var manifest = loadManifest();
  var examples = manifest.tasks.templates.press.examples;
  ...
  examples.forEach(function (ex) {
    assert.ok(ex.voicePrompt, ex.id + ' 应已接线 voicePrompt');
  ...
  assert.equal(checked, 47, '47 条 example 的 ZH 音频应全部逐一验证过');
  console.log('PASS 4: 47 条已交付 example 的 .zh.m4a 均真实存在、非空，且与 tts-text-manifest.zh.json 的 out 字段一致。');
});

test('5. 全部 47 个目标的 EN 整句也已交付：EN(tts-text-manifest.json) 文案就绪 + audio/tasks/press-<key>.m4a 磁盘文件真实存在非空（EN 语言模式的按键任务可用性依据），命名 stem 与 targetKey 推导一致', function () {
  var manifest = loadManifest();
  var examples = manifest.tasks.templates.press.examples;
  ...
  examples.forEach(function (ex) {
    ...
    assert.ok(enEntry, ex.id + ' 应在 tts-text-manifest.json.tasks 里有 EN 文案');
    assert.equal(enEntry.out, 'audio/tasks/' + stem + '.m4a', ex.id + ': EN out 路径应遵循 audio/tasks/' + stem + '.m4a 约定');
    assert.ok(typeof enEntry.text === 'string' && enEntry.text.length > 0, ex.id + ': EN 文案不应为空');

    var zhEntry = zhManifest.tasks[ex.id];
    assert.ok(zhEntry, ex.id + ' 应在 tts-text-manifest.zh.json.tasks 里有 ZH 文案');
    assert.equal(zhEntry.out, 'audio/tasks/' + stem + '.zh.m4a', ex.id + ': ZH out 路径应遵循 audio/tasks/' + stem + '.zh.m4a 约定');
    assert.ok(typeof zhEntry.text === 'string' && zhEntry.text.length > 0, ex.id + ': ZH 文案不应为空');

    // EN 整句音频（.m4a）也应真实交付在磁盘上（no-silent-fallback：EN 语言模式的按键任务可用性
    // 台账 EN_AVAILABLE_TASK_IDS 依赖这些文件真实存在）。
    var enAbs = path.join(APP_WEB, 'audio/tasks/' + stem + '.m4a');
    assert.equal(existsSync(enAbs), true, stem + '.m4a（EN 整句）必须真实存在于磁盘: ' + stem);
    assert.ok(statSync(enAbs).size > 0, stem + '.m4a 不应是 0 字节空文件');
    checked += 1;
  });
  assert.equal(checked, 47, '47 个目标的 EN 整句音频应全部逐一验证过');
  console.log('PASS 5: 47 个目标的 EN 整句音频均真实存在非空，文案/命名约定一致——EN 侧按键任务全量交付。');
});
