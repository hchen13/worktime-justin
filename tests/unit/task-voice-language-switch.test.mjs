...
test('1b. 全部 example 逐一 startTask，接上 voice-language.js 后：32 条已交付的均能 fetch 到各自 .zh.m4a；WTJ-20260706-010 新增的 40 条按键池扩容 example（voicePrompt 待 ASR-gated 生成，TL 已暂停裸生成脚本）0-fetch 空跳过，符合 no-silent-fallback', function () {
  ...
  assert.equal(checked, 72, '四类模板已交付语音的 72 条 example（原 32 + 010 按键池扩容 40 条 ZH 整句）都应被逐一验证过');
  // WTJ-20260706-010：按键池新增的 40 条 example（全部在 press 类型下）的中文整句已由 ASR-gated
  // CosyVoice3 全部交付并接线（voicePrompt 从 '' 改指向各自 .zh.m4a），因此不再有 0-fetch 空跳过
  // 的 pending example（与 025→024 那次"先扩池、voicePrompt 留空，音频到位后再接线"的既有先例
  // 同一模式，此刻已完成接线一步）。
  assert.equal(checkedPending, 0, 'WTJ-20260706-010 按键池 40 条 ZH 整句全部交付接线后，不再有 voicePrompt 空字符串的待交付 example');
  console.log('PASS 1b: 72 条已交付 example 均正确 fetch 到各自 .zh.m4a（含 010 按键池扩容 40 条），无一失声/播错。');
});
