...
test('1b. 全部 example 逐一 startTask，接上 voice-language.js 后：32 条已交付的均能 fetch 到各自 .zh.m4a；WTJ-20260706-010 新增的 40 条按键池扩容 example（voicePrompt 待 ASR-gated 生成，TL 已暂停裸生成脚本）0-fetch 空跳过，符合 no-silent-fallback', function () {
  var checked = 0;
  ...
  ['drag', 'click', 'find', 'press'].forEach(function (type) {
    ...
    examples.forEach(function (ex, idx) {
      ...
      if (!ex.voicePrompt) {
        // WTJ-20260705-025 新增、024/084 尚未交付中文语音的任务：voicePrompt 是空字符串，
        // task.js 的 playTaskVoiceDefensive() 与 voice-language.js 的 extractVoicePrompt()
        // 对它的既有处理是"falsy -> 直接短路，不发起任何 fetch"——这正是本卡刻意选择的
        // no-silent-fallback 落地方式（既不失声报错，也不静默播放不相关的语音）。
        assert.equal(oneEnv.fetchCalls.length, 0, ex.id + '（voicePrompt 为空，待 024/084 交付）应 0 次 fetch，不尝试播放');
        checkedPending += 1;
      } else {
  ...
  assert.equal(checked, 32, '四类模板已交付语音的 32 条 example（原 24 + 024 CosyVoice3 新交付 8）都应被逐一验证过');
  // WTJ-20260706-010：按键池新增的 40 条 example（全部在 press 类型下）当前 voicePrompt 均为
  // 空字符串——TL 已暂停裸 generate-tts-cosyvoice3.py 生成，改走共享 ASR-gated wrapper（字母
  // C/G/J/K/Q/W/Y 与符号名误读风险高），音频生成 + manifest.js 接线是本卡的后续 commit。这条
  // 断言镜像 025→024 那次"先扩池、voicePrompt 留空，音频到位后再接线"的既有先例（当时是 8 条，
  // 见本用例历史标题），40 是本次的临时中间态数字，音频接线完成后应回落到 0。
  assert.equal(checkedPending, 40, 'WTJ-20260706-010 按键池扩容的 40 条 example 应 0-fetch 空跳过（voicePrompt 待 ASR-gated 生成交付）');
  console.log('PASS 1b: 32 条已交付 example 均正确 fetch 到各自 .zh.m4a；010 新增 40 条按键池扩容 example（voicePrompt 待生成）均 0-fetch 空跳过，无一失声/播错。');
});
