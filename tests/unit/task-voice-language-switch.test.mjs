...
test('1b. 全部 example 逐一 startTask，接上 voice-language.js 后：24 条已交付的均能 fetch 到各自 .zh.m4a；WTJ-20260705-025 新增的 8 条（voicePrompt 为空字符串，024/084 未交付）应 0 次 fetch，安静跳过而不是失声报错', function () {
  var checked = 0;
  ...
  assert.equal(checked, 24, '四类模板已交付语音的 24 条 example 都应被逐一验证过');
  assert.equal(checkedPending, 8, 'WTJ-20260705-025 新增的 8 条待交付 example（6 条 drag + 2 条 click）都应被逐一验证过');
  console.log('PASS 1b: 24 条已交付 example 均正确 fetch 到各自 .zh.m4a，8 条新增待交付 example 均 0 次 fetch 安静跳过，无一失声/播错。');
});
