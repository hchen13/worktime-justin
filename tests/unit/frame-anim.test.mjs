...
// =============================================================================================
// WTJ-20260707-007 — 首帧透明不算「画过」（door/bell 旧机不显示的真实根因，014 残留缺陷）
// =============================================================================================
// 背景（详见 frame-anim.js 文件头「WTJ-20260707-007」一节）：isEntryReady() 的同步兜底判据
// `img.complete && naturalWidth>0` 在 WebKit「字节已加载完、位图尚未解码完成」的窗口里就返回
// true，此时 drawImage 静默画出全透明。014 的 drawFrame() 只要 drawImage 不抛错就置 hasDrawnOnce，
// 于是单帧快路径（door.closed / bell.idle 等）一旦在这个窗口里画了一次透明，就永久停止重试、
// canvas 永久空白——这正是 Justin 旧机上门/门铃「不显示」的根因（资产/映射/命中链路都是对的）。
// 修复：drawFrame() 用 getImageData 采样确认首帧真的落了非透明像素，透明则保持 hasDrawnOnce=false
// 继续重试，直到解码完成画出真内容为止。下面用一个「可采样的 fake canvas」精确复现这条路径。
//
// 这个 fake ctx 有 getImageData（既有 makeCanvas() 的 ctx 故意没有——那模拟「环境无法采样、按
// 可信放行」的兜底分支，恰好让上面全部既有单测在本修复下行为不变）；hasContent 标志由测试手动
// 翻转，模拟「isEntryReady 已 true（onload 已到）但位图要更晚才解码完、drawImage 期间画出的是
// 透明 → 稍后解码完成、drawImage 才画出真像素」这一真机时序。
function makeSamplableCanvas() {
  var ctx2d = makeFakeCtx2D();
  var hasContent = false; // false=drawImage 画出全透明（解码未完成）；true=画出真像素（解码完成）
  ctx2d.getImageData = function (x, y, gw, gh) {
    var n = gw * gh * 4;
    var data = new Uint8ClampedArray(n); // 默认全 0 = 全透明
    if (hasContent) {
      data[3] = 255; // 至少一个非透明像素，代表这一帧真的画上了内容
    }
    return { data: data };
  };
  var el = {
    ...
    height: 0,
    ctx2d: ctx2d,
    getContext: function (type) { return type === '2d' ? ctx2d : null; },
    setHasContent: function (v) { hasContent = !!v; }
  };
  return el;
}

test('WTJ-20260707-007：单帧素材（bell.idle）isEntryReady 已 true 但首帧画出全透明（位图未解码完）时，不得把 hasDrawnOnce 置真、必须持续重试；直到某一拍画出真像素才认定画过并停止', function () {
  var env = createSandbox();
  var canvas = makeSamplableCanvas();

  // loop:true 避免额外的 onComplete 定时器混进受控时钟队列（同 ① 用例的理由）。
  env.FA.play(canvas, 'bell', 'idle', { loop: true });
  // 图片尚未 ready：走单帧 retryDrawUntilReady 快路径，未就绪不画、排一个重试定时器。
  assert.equal(canvas.ctx2d.drawImageCalls.length, 0, '未就绪时不应发生 drawImage');
  assert.equal(env.clock.pendingCount(), 1, '未就绪时应排一个重试定时器');

  // 关键：markReady 让 isEntryReady 返回 true（onload → complete && naturalWidth），但 canvas 的
  // hasContent 仍为 false——模拟「引擎认为就绪、drawImage 却画出全透明（位图尚未解码）」的窗口。
  env.imgEnv.markReady(env.imgEnv.lastCreated());

  env.clock.fireAt(16);
  assert.equal(canvas.ctx2d.drawImageCalls.length, 1, '就绪后这一拍会 drawImage（但画的是透明）');
  assert.equal(env.FA.getState().activePlaybacks[0].hasDrawnOnce, false,
    '首帧画出的是全透明——绝不能据此认定「画过一次」（旧实现在这里就永久停了，正是本卡根因）');
  assert.equal(env.clock.pendingCount(), 1, '首帧透明：必须继续排下一次重试，不能停');

  // 再来两拍，位图仍未解码完（hasContent 仍 false）：每拍重画、每拍仍是透明、每拍都继续重试。
  env.clock.fireAt(32);
  env.clock.fireAt(48);
  assert.equal(canvas.ctx2d.drawImageCalls.length, 3, '透明期间每一拍都重画一次');
  assert.equal(env.FA.getState().activePlaybacks[0].hasDrawnOnce, false, '一直透明就一直不算画过');
  assert.equal(env.clock.pendingCount(), 1, '一直透明就一直继续重试（door/bell 旧机不显示的修复核心）');

  // 位图终于解码完成：下一拍 drawImage 画出真像素，getImageData 采到非透明 → 认定画过、停止重试。
  canvas.setHasContent(true);
  env.clock.fireAt(64);
  assert.equal(canvas.ctx2d.drawImageCalls.length, 4, '解码完成这一拍再画一次，这次画出真像素');
  assert.equal(env.FA.getState().activePlaybacks[0].hasDrawnOnce, true, '画出真像素后才认定 hasDrawnOnce');
  assert.equal(env.clock.pendingCount(), 0, '单帧画出真内容后收敛，不再排任何定时器');
});

test('WTJ-20260707-007：环境无法采样（fake ctx 无 getImageData）时按可信放行——既有 makeCanvas() 单测行为不回归（首帧 drawImage 一次即认定画过并收敛）', function () {
  var env = createSandbox();
  var canvas = makeCanvas(); // 无 getImageData 的既有 stub → firstPaintDepositedContent() 返回 true
  env.FA.play(canvas, 'bell', 'idle', { loop: true });
  env.imgEnv.markReady(env.imgEnv.lastCreated());
  env.clock.fireAt(16);
  assert.equal(canvas.ctx2d.drawImageCalls.length, 1, '就绪后画一次');
  assert.equal(env.FA.getState().activePlaybacks[0].hasDrawnOnce, true,
    '无法采样时不应因此把画好的帧误判为空白——按可信放行，与 014 既有语义一致');
  assert.equal(env.clock.pendingCount(), 0, '单帧画出后收敛');
});
