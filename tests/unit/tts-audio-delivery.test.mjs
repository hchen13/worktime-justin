...
test('074+015+008+010: expected delivered TTS counts (100 words delivered incl. fox / 53 EN tasks / 10 phrases)', () => {
  ...
  // WTJ-20260706-010: keypress pool expansion added 45 EN whole-sentence press prompts
  // (all 47 targets minus press-a/press-3 already delivered), so EN taskVoice 8 -> 53.
  assert.equal(words.length, 100, '100 secret word audio files delivered (incl. fox via 008 ASR-gated rework)');
  assert.equal(tasks.length, 53, 'all 53 EN task prompts delivered (8 original + 45 WTJ-010 keypress-pool EN sentences)');
...
test('084+024+010: expected delivered ZH task-voice count (72: 24 + 025 new 8 + 010 keypress-pool 40)', () => {
  assert.equal(tasksZh.length, 72, 'all 72 ZH task sentences delivered (24 + WTJ-025 new 8 + WTJ-010 keypress-pool 40, ASR-gated CosyVoice3)');
});
