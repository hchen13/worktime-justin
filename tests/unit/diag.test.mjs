...
// =============================================================================================
// 7. rAF ticking + 心跳（heartbeat）
// =============================================================================================

test('rAF ticking：跑 3 帧后触发心跳，记录 rafTicksSinceLast=3 且 rafTicking=true；随后无新帧再触发心跳应为 false', function () {
  var frameAnimStub = {
    getState: function () {
      return { availableProps: ['faucet', 'horse', 'lamp'], idleStopSec: 5, activePlaybacks: [{ prop: 'faucet', state: 'idle', hasDrawnOnce: true }] };
    }
  };
  var env = createSandbox({ frameAnim: frameAnimStub });
