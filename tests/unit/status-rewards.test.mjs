...
// ============================================================================================
// WTJ-20260706-013：status-rewards.css 静态契约：prefers-reduced-motion 兜底块内每条规则的
...
// 属性，这条门使整段 reduced-motion 覆盖不生效，completion-stamp-v3 与里程碑贴纸的一次性
// pop/scale/fade 动画照常播放；只有家长设置把 honorReducedMotion 翻回 true（不打属性）时才
// 恢复 OS reduced-motion 覆盖。本文件 status-rewards.js 自身的 prefersReducedMotion() 判定
// （feed 进 onWorkComplete payload 的 reducedMotion 字段供 QA 断言）未受 013 影响，继续如实
// 反映 matchMedia 命中结果（见既有用例"prefers-reduced-motion 命中时"），本测试只补 CSS
// 侧的门。
// ============================================================================================
test('013 status-rewards.css 静态契约：prefers-reduced-motion 兜底块内 .wtj-sr-stamp/.wtj-sr-milestone-sticker 规则均应加 html:not([data-wtj-motion-forced]) 门', function () {
  var css = readFileSync(SR_CSS_PATH, 'utf8');
  var reducedMotionBlockMatch = css.match(/@media \(prefers-reduced-motion: reduce\)\s*\{([\s\S]*)\}\s*$/);
  assert.ok(reducedMotionBlockMatch, 'status-rewards.css 应包含 prefers-reduced-motion 的兜底块');
  var blockBody = reducedMotionBlockMatch[1];

  ['.wtj-sr-stamp', '.wtj-sr-milestone-sticker'].forEach(function (selector) {
  ...
  var bareMatch = blockBody.match(/(^|[,{};]\s*)\.wtj-sr-(stamp|milestone-sticker)\b/);
  assert.equal(bareMatch, null, '兜底块内不应该存在未加 html:not([data-wtj-motion-forced]) 门的裸选择器，命中：' + (bareMatch && bareMatch[0]));
});
