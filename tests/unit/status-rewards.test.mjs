...
// includeAudio/includeMatchMedia/includeSparkles: false 时不把对应依赖挂到 window 上（模拟
// 缺失场景）。opts.reducedMotion: true 时 window.matchMedia('(prefers-reduced-motion: reduce)')
// .matches 返回 true。opts.innerWidth/innerHeight：WTJ-20260705-015 任务成功 FX 用
// window.innerWidth/innerHeight 把 anchor 的百分比换算成像素坐标，默认 1000x800（好算的整数，
// 方便测试断言精确像素值）。
...
// WTJ-20260705-015：找到挂在 document.body 下的任务成功 FX 画布（懒创建单例，见
// status-rewards.js ensureFxCanvas()）。
function fxCanvas(env) {
  return env.contextObject.document.body.children.filter(function (el) {
    return el.className === 'wtj-sr-taskfx-canvas';
  })[0];
}
...
// ============================================================================================
// WTJ-20260705-015：任务成功即时视觉反馈（第三路，与上方 008/010 两路奖励并列）。
// 每次 onTaskComplete 都应该触发一次短促的 canvas sparkle burst + 成功环，独立于 streak/
// celebrating 状态机——不因为正在庆祝三连奖励而跳过，也不因为不足 3 个而不播放。
// ============================================================================================

test('015：每次 onTaskComplete 都会懒创建 .wtj-sr-taskfx-canvas 画布并挂到 document.body（独立于 008/010 两路奖励叠层的 root）', function () {
  var env = createSandbox();
  assert.equal(fxCanvas(env), undefined, '触发前不应该创建任务成功 FX 画布');

  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 50, topPercent: 50 } });

  var canvas = fxCanvas(env);
  assert.ok(canvas, '任务完成后应该创建 .wtj-sr-taskfx-canvas 画布');
  assert.equal(canvas.tagName, 'canvas');

  var canvasCountAfterFirst = env.contextObject.document.body.children.filter(function (el) {
    return el.className === 'wtj-sr-taskfx-canvas';
  }).length;
  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'b', lightIndex: 1, anchor: { leftPercent: 20, topPercent: 30 } });
  var canvasCountAfterSecond = env.contextObject.document.body.children.filter(function (el) {
    return el.className === 'wtj-sr-taskfx-canvas';
  }).length;
  assert.equal(canvasCountAfterSecond, canvasCountAfterFirst, '画布应该是懒创建单例，第二次任务完成不应该再创建一个新画布');
});

test('015：anchor 为 {leftPercent, topPercent} 时，成功环锚点按 window.innerWidth/innerHeight 换算成正确像素坐标', function () {
  var env = createSandbox({ innerWidth: 1000, innerHeight: 800 });

  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 38, topPercent: 16 } });

  var canvas = fxCanvas(env);
  assert.ok(canvas.ctx2d.arcCalls.length >= 1, '应该至少画出一次成功环（ctx.arc 被调用）');
  var firstArc = canvas.ctx2d.arcCalls[0];
  assert.equal(firstArc.x, 380, '锚点 x 应为 leftPercent/100 * innerWidth = 0.38 * 1000 = 380');
  assert.equal(firstArc.y, 128, '锚点 y 应为 topPercent/100 * innerHeight = 0.16 * 800 = 128');

  // 星点炸开同样应该以同一个像素锚点为中心（drawSparkles 的 x/y 入参）。
  assert.ok(env.sparklesStub.calls.length >= 1, '应该调用过 window.WTJ_SPARKLES.drawSparkles');
  assert.equal(env.sparklesStub.calls[0].x, 380, 'drawSparkles 的 x 入参应与成功环同一个像素锚点');
  assert.equal(env.sparklesStub.calls[0].y, 128, 'drawSparkles 的 y 入参应与成功环同一个像素锚点');
});

test('015：press 类任务没有 DOM，anchor 为 null 时回退到画布安全区 FALLBACK_ANCHOR_PCT（屏幕偏上方居中）', function () {
  var env = createSandbox({ innerWidth: 1000, innerHeight: 800 });
  ...
  var canvas = fxCanvas(env);
  var firstArc = canvas.ctx2d.arcCalls[0];
  // FALLBACK_ANCHOR_PCT = { leftPercent: 50, topPercent: 40 }（见 status-rewards.js 常量）。
  assert.equal(firstArc.x, 500, 'press 类任务应回退到画布安全区，x = 50% * 1000 = 500');
  assert.equal(firstArc.y, 320, 'press 类任务应回退到画布安全区，y = 40% * 800 = 320');
});

test('015：sparkle burst 是短促的分步动画——8 个方向 * 多个步数，alpha 逐步淡出到 0；推进完整时长后画布被清空', function () {
  var env = createSandbox();
  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 50, topPercent: 50 } });

  var canvas = fxCanvas(env);
  var firstStepCallCount = env.sparklesStub.calls.length;
  assert.equal(firstStepCallCount, 8, '第一步应该朝 8 个方向各调用一次 drawSparkles（FX_SPARKLE_DIRECTIONS=8）');
  var firstStepAlphas = env.sparklesStub.calls.slice(0, 8).map(function (c) { return c.opts.alpha; });
  assert.ok(firstStepAlphas.every(function (a) { return a === firstStepAlphas[0]; }), '同一步内 8 个方向应使用相同的 alpha（同一帧内的整体强度包络一致）');
  assert.equal(firstStepAlphas[0], 1, '第一步的 alpha 应为 1（burst 最明显的起始帧）');

  env.clock.advance(FULL_TASKFX_BURST_MS);

  // 最后一步 alpha 应该淡到 0（或非常接近 0）——按线性淡出公式 alpha = 1 - step/(N-1)。
  var lastStepAlpha = env.sparklesStub.calls[env.sparklesStub.calls.length - 1].opts.alpha;
  assert.ok(lastStepAlpha <= 0.15, '最后一步 alpha 应该已经淡出到接近 0（短促、不过载）');

  // burst 结束后画布应该被清空（不常驻可见内容）。
  assert.ok(canvas.ctx2d.clearRectCallCount() > 0, 'burst 结束后应该调用过 ctx.clearRect 清空画布');
});

test('015：无条件触发——即使 celebrating 期间（第 3 个任务同时触发三灯庆祝），任务成功 FX 仍然照常播放，不受并发守卫影响', function () {
  var env = createSandbox();
  ...
  var callsBeforeThird = env.sparklesStub.calls.length;

  // 第 3 次：既触发「今日工作完成」三灯庆祝，也应该照常触发这一次任务自己的即时 FX。
  ...
  assert.ok(env.sparklesStub.calls.length > callsBeforeThird, '即使这一次同时触发了三灯庆祝，任务成功 FX 也应该照常播放（不受 celebrating 并发守卫影响）');

  // celebrating 期间被吞掉的第 4 次任务完成（不计入 streak），FX 仍然应该照常播放——这是"这一次
  // 任务真的完成了"的即时反馈，与是否计入下一轮 streak 是两件独立的事。
  var callsBeforeFourth = env.sparklesStub.calls.length;
  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'd', lightIndex: 0, anchor: { leftPercent: 40, topPercent: 40 } });
  assert.ok(env.sparklesStub.calls.length > callsBeforeFourth, 'celebrating 期间被并发守卫吞掉 streak 计数的任务完成，FX 仍应该照常播放');
});

test('015：prefers-reduced-motion 命中时，画一帧静态终态（不做分步动画），短暂展示后清空', function () {
  var env = createSandbox({ reducedMotion: true });

  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 50, topPercent: 50 } });

  var canvas = fxCanvas(env);
  assert.equal(canvas.ctx2d.arcCalls.length, 1, 'reduced-motion 下应该只画一帧（不做分步动画）');
  assert.equal(canvas.ctx2d.clearRectCallCount(), 0, '静态帧展示期间不应该立即清空');

  env.clock.advance(1000);
  assert.ok(canvas.ctx2d.clearRectCallCount() > 0, '静态帧短暂展示后应该被清空（JS 定时移除窗口不变，只是不做分步动画）');
});

test('015：reset() 立即中止进行中的 burst 并清空画布，之后推进虚拟时间不应该再有残留的绘制调用', function () {
  var env = createSandbox();

  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 50, topPercent: 50 } });
  var canvas = fxCanvas(env);
  var arcCallsBeforeReset = canvas.ctx2d.arcCalls.length;
  assert.ok(arcCallsBeforeReset >= 1, 'reset 前应该已经画过至少一帧');

  env.SR.reset();
  assert.ok(canvas.ctx2d.clearRectCallCount() > 0, 'reset() 应该立即清空任务成功 FX 画布');

  var arcCallsAfterReset = canvas.ctx2d.arcCalls.length;
  env.clock.advance(FULL_TASKFX_BURST_MS);
  assert.equal(canvas.ctx2d.arcCalls.length, arcCallsAfterReset, 'reset() 应该中止进行中的 burst 定时器链，之后推进虚拟时间不应该再有新的绘制调用');
});

test('015：window.WTJ_SPARKLES 缺失时，成功环本体仍正常绘制（不依赖 sparkles.js），星点炸开效果静默跳过且有一条防御式 console.warn', function () {
  var env = createSandbox({ includeSparkles: false });
  assert.ok(
    env.warnCalls.some(function (m) { return m.indexOf('WTJ_SPARKLES') !== -1; }),
    '应该有一条关于 window.WTJ_SPARKLES 未找到的 console.warn（防御式降级，成功环仍会正常绘制）'
  );

  env.ttStub.fireTaskComplete({ type: 'click', taskId: 'a', lightIndex: 0, anchor: { leftPercent: 50, topPercent: 50 } });

  var canvas = fxCanvas(env);
  assert.ok(canvas.ctx2d.arcCalls.length >= 1, '成功环（ctx.arc/stroke）不依赖 WTJ_SPARKLES，缺失时仍应正常绘制');
  assert.ok(canvas.ctx2d.strokeCallCount() >= 1, '成功环应该调用过 ctx.stroke()');
});

test('015：无 document（非浏览器测试沙箱）时不抛错——防御式跳过画布创建', function () {
  assert.doesNotThrow(function () {
    var warnCalls = [];
    var errorCalls = [];
    var ttStub = makeTaskTemplatesStub();
    var fakeWindow = { WTJ_TASK_TEMPLATES: ttStub.api, innerWidth: 1000, innerHeight: 800 };
    var sandbox = {
      window: fakeWindow,
      console: {
        log: function () {},
        warn: function () { warnCalls.push(1); },
        error: function () { errorCalls.push(1); }
      }
    };
    vm.createContext(sandbox);
  ...
  }, 'document 完全缺失时任务成功 FX 应该静默跳过画布创建，不应该抛错');
});

test('015 status-rewards.css 静态契约：.wtj-sr-taskfx-canvas 存在，z-index:7（高于任务叠层 6，低于秘密词命中 8/工作完成 9），pointer-events:none 不拦截鼠标', function () {
  var css = readFileSync(SR_CSS_PATH, 'utf8');
  var block = css.match(/\.wtj-sr-taskfx-canvas\s*\{[^}]*\}/);
  assert.ok(block, 'status-rewards.css 应定义 .wtj-sr-taskfx-canvas 规则块');
  assert.ok(/z-index:\s*7\b/.test(block[0]), '.wtj-sr-taskfx-canvas 应为 z-index:7');
  assert.ok(/pointer-events:\s*none/.test(block[0]), '.wtj-sr-taskfx-canvas 应 pointer-events:none，不拦截鼠标/指针交互');
});
