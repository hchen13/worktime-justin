...
// includeAudio/includeMatchMedia: false 时不把对应依赖挂到 window 上（模拟缺失场景）。
...
test('第 3 个任务完成触发「今日工作完成」：onWorkComplete emit + 三灯闪烁 + 奖励叠层出现', function () {
  ...
  assert.ok(workCompletePayloads[0].forms.indexOf('mini-rocket-launch') !== -1, 'forms 应包含 mini-rocket-launch');
  ...
  assert.ok(overlayRootAtTrigger.children.length > 0, '触发瞬间叠层应该已经有子元素（火箭/sparkle/星星贴纸）');
...
test('并发守卫：celebrating 期间的任务完成被吞掉——streak 不叠加、不出现第二套奖励叠层', function () {
  ...
  assert.ok(childCountBefore > 0, '庆祝期间叠层应该已经有子元素（火箭/sparkle/星星）');
