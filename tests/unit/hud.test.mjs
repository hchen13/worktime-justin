...
function createSandbox(opts) {
  ...
  // WTJ-20260705-019b：提供真实 setTimeout/clearTimeout——本文件多数用例不加载 secretword.js，
  // hud.js 顶层的 wireTerminalSecretWordDeferred() 会走"window.WTJ_SECRET 不可用"这条更干净
  // 的防御式 console.warn 分支（而不是缺 setTimeout 时那条更吵的 ReferenceError 捕获日志），
  // 不影响本文件任何既有断言（该延后订阅本身是防御式的，找不到 WTJ_SECRET 时静默降级）。
  var sandbox = {
    window: fakeWindow,
...
test('7. window.WTJ_HUD 是冻结对象，API 方法齐全（含 WTJ-20260704-083 返工新增的 setChestOpen）', function () {
  ...
  ['setSlot', 'clearSlots', 'setStatusLight', 'onQuestionClick', 'setChestOpen', 'getState', '_setClock'].forEach(function (name) {
...
// ============================================================================================
// 14.（WTJ-20260705-019b，Ethan 截图反馈①：header 双语标题居中）DOM 结构断言——标题组是两个
//     独立文本节点（英文主标题 + 中文副标题），且真实渲染的英文/中文文本内容正确。
// ============================================================================================
function topbarEl(sb) {
  var root = hudRoot(sb);
  return root.children.filter(function (el) { return el.className === 'wtj-hud-topbar'; })[0];
}

test('14. buildTopbar() 渲染标题组（.wtj-hud-title-group），内含英文主标题 + 中文副标题两个独立文本节点，文案正确', function () {
  var sb = createSandbox();
  var topbar = topbarEl(sb);
  assert.ok(topbar, '应能找到 .wtj-hud-topbar');

  var titleGroup = topbar.children.filter(function (el) { return el.className === 'wtj-hud-title-group'; })[0];
  assert.ok(titleGroup, '应存在 .wtj-hud-title-group 标题组容器');

  var titleEn = titleGroup.children.filter(function (el) { return el.className === 'wtj-hud-title-en'; })[0];
  assert.ok(titleEn, '应存在 .wtj-hud-title-en 英文主标题节点');
  assert.equal(titleEn.textContent, 'Work Time, Justin!');

  var titleZh = titleGroup.children.filter(function (el) { return el.className === 'wtj-hud-title-zh'; })[0];
  assert.ok(titleZh, '应存在 .wtj-hud-title-zh 中文副标题节点');
  assert.equal(titleZh.textContent, '小小工作台');

  // 旧的单一 .wtj-hud-title 节点不应再出现（已拆分为上面两个节点，不是新旧并存）。
  var oldTitle = topbar.children.filter(function (el) { return el.className === 'wtj-hud-title'; })[0];
  assert.equal(oldTitle, undefined, '不应再渲染旧的单行 .wtj-hud-title 节点');

  // 锁形 glyph 仍然是 topbar 的直接子节点（结构不变，只是 CSS 定位从 flex 布局改为 absolute）。
  var lock = topbar.children.filter(function (el) { return el.className === 'wtj-hud-lock'; })[0];
  assert.ok(lock, '锁形 glyph 仍应是 topbar 的直接子节点');
});
...
test('15. hud.css 静态契约：--generic tray-wrap 宽度显著放宽（不再是 clamp(260px,30vw,380px) 小方块），槽径改用与容器尺寸解耦的 clamp()', function () {
  ...
  assert.equal(/width:\s*24%/.test(genericSlotMatch[0]), false, '槽径不应再是相对容器高度的 24% 百分比（容器不再是正方形，百分比会失真）');
  assert.match(genericSlotMatch[0], /width:\s*clamp\(/, '槽径应改用与容器尺寸解耦的绝对 clamp() 直径');

  // footer 本体（不是 footer-bar 小胶囊）现在才是承载"高对比背景"这个视觉的容器，见测试 12。
  var footerMatch = css.match(/\.wtj-hud-footer\s*\{[^}]*\}/);
  assert.ok(footerMatch, '应能找到 .wtj-hud-footer 规则块');
  assert.match(footerMatch[0], /width:\s*100%/, 'footer 本体仍应声明 width:100%（全宽契约，回归门叠加测试 13）');
});

// ============================================================================================
// 16.（WTJ-20260705-019b，Ethan 截图反馈③：terminal prompt 用 justin@worktime: 前缀样式）
//     DOM 结构断言——buildTerminalPrompt() 渲染 prefix/word/cursor/pip 四个节点，idle 态 word
//     为空字符串（不是输入框、不做逐键回显）。
// ============================================================================================
function terminalWrapEl(sb) {
  var root = hudRoot(sb);
  return root.children.filter(function (el) { return el.className === 'wtj-hud-terminal'; })[0];
}

test('16. buildTerminalPrompt() 渲染 justin@worktime: 前缀 + 空的秘密词展示区 + 光标 + activity pip；idle 态不含任何输入框/回显内容', function () {
  var sb = createSandbox();
  var terminal = terminalWrapEl(sb);
  assert.ok(terminal, '应能找到 .wtj-hud-terminal 根节点');

  var glyph = terminal.children.filter(function (el) { return el.className === 'wtj-hud-terminal-glyph'; })[0];
  assert.ok(glyph, '应存在 .wtj-hud-terminal-glyph 容器');

  var prefix = glyph.children.filter(function (el) { return el.className === 'wtj-hud-terminal-prefix'; })[0];
  assert.ok(prefix, '应存在 .wtj-hud-terminal-prefix 前缀节点');
  assert.equal(prefix.textContent, 'justin@worktime:', '前缀文本应恰好是 "justin@worktime:"');

  var word = glyph.children.filter(function (el) { return el.className === 'wtj-hud-terminal-word'; })[0];
  assert.ok(word, '应存在 .wtj-hud-terminal-word 秘密词展示节点');
  assert.equal(word.textContent, '', 'idle 态秘密词展示区应为空字符串（不是逐键回显，也不是预置占位词）');

  var cursor = glyph.children.filter(function (el) { return el.className === 'wtj-hud-terminal-cursor'; })[0];
  assert.ok(cursor, '应存在 .wtj-hud-terminal-cursor 光标节点');
  assert.equal(cursor.textContent, '_');

  var pip = terminal.children.filter(function (el) { return el.className === 'wtj-hud-terminal-pip'; })[0];
  assert.ok(pip, '应存在 .wtj-hud-terminal-pip 装饰圆点');
  assert.equal(pip.textContent, '', 'activity pip 不应有任何文本');

  // 旧的 arrow 节点（`>` 字符）不应再出现——已被 justin@worktime: 前缀取代，不是新旧并存。
  var oldArrow = glyph.children.filter(function (el) { return el.className === 'wtj-hud-terminal-arrow'; })[0];
  assert.equal(oldArrow, undefined, '不应再渲染旧的 .wtj-hud-terminal-arrow（`>` 字符）节点');
});

// ============================================================================================
// 17.（WTJ-20260705-019b，Ethan 截图反馈③：秘密词完成时在 terminal 显示拼写，且与对应 sprite
//     同步出现）真实集成测试——在同一沙箱里按 index.html 真实顺序加载 manifest.js ->
//     （keyboard 打桩）-> hud.js -> secretword.js，驱动一次真实的 'd','o','g' 字母流，断言：
//       a) secretword.js 的 sprite 一次性叠层（.wtj-secret-sprite）与 hud.js 的 terminal 秘密词
//          展示（.wtj-hud-terminal-word）在同一次同步的 handleHit() 调用内都出现——不是"terminal
//          先出现，sprite 隔了一拍才出现"这种割裂观感，也不是只做了其中一个。
//       b) terminal 展示的文本内容就是命中词本身（"dog"），不是空壳/占位符。
//       c) HUD._setClock() 注入假时钟快进 TERMINAL_WORD_DISPLAY_MS 之后，展示区自动清空回
//          idle 态（不会永久停留在"dog"，仍然只是一次性完成态展示，不是新增的常驻回显）。
//     跨脚本加载顺序（secretword.js 在 index.html 里晚于 hud.js 加载）用真实的
//     setTimeout(fn,0) 宏任务验证 hud.js wireTerminalSecretWordDeferred() 的宏任务延后订阅
//     手法在真实 Node 事件循环时序下确实可行，不是仅在同步执行下"看起来对"。
// ============================================================================================
...
// 与 task-templates.test.mjs / task-lifecycle.test.mjs 的 makeFakeClock() 同一实现手法：
// 供 HUD._setClock() 注入，让 TERMINAL_WORD_DISPLAY_MS（1900ms）的自动清空可以用 advance(ms)
// 手动快进虚拟时间断言，不需要真的等待。
function makeFakeClock() {
  ...
  return { setTimeout: fakeSetTimeout, clearTimeout: fakeClearTimeout, advance: advance };
}

// 构建 manifest -> keyboard 打桩 -> hud.js -> secretword.js 的集成沙箱。keyboard 打桩与
// tests/unit/secretword-engine.test.mjs 的 fakeWindow.WTJ_KEYBOARD 同一手法：只捕获
// onLetter(fn) 注册的处理函数供测试手动调用注入字母流，不依赖真实 keyboard.js 的 DOM 事件/
// 节奏门控逻辑（那部分已由 keyboard.test.mjs 独立覆盖）；同时提供 onEffectiveKey 空实现，
// 满足 hud.js wireTerminalKeyActivity() 的可选订阅（本测试不断言按键活跃度微亮）。
// 顶层 setTimeout/clearTimeout 用真实 Node 版本（不是假时钟）——hud.js
// wireTerminalSecretWordDeferred() 依赖真实的 0ms 宏任务时序才能验证"确实晚于 secretword.js
// 加载完成后再订阅"，假时钟不适用于这一段基础设施性质的延后逻辑（见 hud.js 该函数顶部注释）。
function createIntegrationSandbox() {
  var docStub = makeFakeDocument();
  var letterHandlers = [];
  var fakeWindow = {
    location: { search: '' },
  ...
  return {
    ...
    body: docStub.body,
    emitLetter: emitLetter
  };
}

test('17. 秘密词完成时 terminal 显示拼写且与 sprite 同步出现（真实 handleHit() 同步调用），假时钟快进后自动清空回 idle', async function () {
  var sb = createIntegrationSandbox();
  assert.ok(sb.SECRET, '前置检查：secretword.js 应成功加载并暴露 window.WTJ_SECRET');

  // hud.js 顶层的 wireTerminalSecretWordDeferred() 用 setTimeout(fn, 0) 延后到"本文档内所有
  // 同步 <script> 都执行完毕"之后再订阅 window.WTJ_SECRET.onHit（diag.js 同款宏任务延后
  // 手法）——在真实 Node 事件循环里，等价于这里也真的经过一次 0ms 宏任务。
  await new Promise(function (resolve) { setTimeout(resolve, 0); });

  var fakeClock = makeFakeClock();
  sb.HUD._setClock({ setTimeout: fakeClock.setTimeout, clearTimeout: fakeClock.clearTimeout });

  var terminal = terminalWrapEl(sb);
  var glyph = terminal.children.filter(function (el) { return el.className === 'wtj-hud-terminal-glyph'; })[0];
  var word = glyph.children.filter(function (el) { return el.className === 'wtj-hud-terminal-word'; })[0];
  assert.equal(word.textContent, '', '前置检查：命中前 terminal 秘密词展示区应为空');

  // 驱动真实字母流 'd' -> 'o' -> 'g'，走真实的 secretword.js tryMatchAtBufferTail()/handleHit()。
  sb.emitLetter('d');
  sb.emitLetter('o');
  sb.emitLetter('g');

  // a) sprite 一次性叠层与 terminal 秘密词展示应该都已出现——同一次同步 handleHit() 调用内
  //    先后调用 showSpriteOverlay() 与 emit(hitSubscribers,...)，二者在这里都应已是"已发生"。
  var spriteImgs = sb.body.children.filter(function (el) { return el.className === 'wtj-secret-overlay'; });
  assert.ok(spriteImgs.length > 0, '应能找到 secretword.js 的 sprite 叠层容器（.wtj-secret-overlay）');
  var spriteImg = spriteImgs[0].children.filter(function (el) { return el.className === 'wtj-secret-sprite'; })[0];
  assert.ok(spriteImg, 'sprite 叠层容器内应有一个 .wtj-secret-sprite <img>（与 terminal 展示同一次 handleHit() 调用内出现，验证"同步"）');

  // b) terminal 展示的确实是命中词本身，不是空壳。
  assert.equal(word.textContent, 'dog', 'terminal 秘密词展示区应显示命中词 "dog"');
  assert.equal(word.classList.contains('is-visible'), true, '秘密词展示节点应挂 is-visible 类（触发 pop 动画）');
  assert.equal(terminal.classList.contains('is-word-pulse'), true, '.wtj-hud-terminal 根节点应挂 is-word-pulse（比逐键活跃度更明显的高亮）');

  // c) 假时钟快进超过 TERMINAL_WORD_DISPLAY_MS（1900ms）后，应自动清空回 idle——仍然只是
  //    一次性完成态展示，不是新增的常驻回显。
  fakeClock.advance(2000);
  assert.equal(word.textContent, '', '快进超过展示窗口后，秘密词展示区应自动清空回空字符串');
  assert.equal(word.classList.contains('is-visible'), false, '快进后 is-visible 应被移除');
  assert.equal(terminal.classList.contains('is-word-pulse'), false, '快进后 .wtj-hud-terminal 的 is-word-pulse 应被移除');

  // getRoundHits() 交叉验证：secretword.js 侧也确实记录了这次命中（不是 terminal 展示与实际
  // 引擎状态两张皮）。
  assert.deepEqual(Array.from(sb.SECRET.getRoundHits()), ['dog']); // Array.from()：跨 vm realm 数组，见本文件其它测试同款处理
});

test('17b. 连续命中两个不同词：terminal 秘密词展示区应更新为最新命中词（重新触发 pop 动画），而不是停留在第一个词', async function () {
  var sb = createIntegrationSandbox();
  await new Promise(function (resolve) { setTimeout(resolve, 0); });
  var fakeClock = makeFakeClock();
  sb.HUD._setClock({ setTimeout: fakeClock.setTimeout, clearTimeout: fakeClock.clearTimeout });
  ...
  sb.emitLetter('o');
  sb.emitLetter('g');
  assert.equal(word.textContent, 'dog');

  // 命中 dog 后（未等展示窗口结束）紧接着命中 cat：terminal 应立即切换展示为最新词，而不是
  // 排队/停留在旧词上（与 sprite 叠层"总是显示最新命中对象"的观感一致）。
  fakeClock.advance(500); // 仍在 1900ms 展示窗口内
  sb.emitLetter('c');
  sb.emitLetter('a');
  sb.emitLetter('t');
  assert.equal(word.textContent, 'cat', 'terminal 应更新为最新命中词 "cat"，而不是停留在 "dog"');
  assert.equal(word.classList.contains('is-visible'), true);
