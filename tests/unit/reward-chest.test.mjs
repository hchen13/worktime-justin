...
// ============================================================================================
// 18.（WTJ-20260706-013）reward-chest.css 静态契约：prefers-reduced-motion 兜底块内每条规则的
...
// 属性，这条门使整段 reduced-motion 覆盖不生效，开箱本体的旋转/缩放动画与背景光晕闪烁照常
// 播放；只有家长设置把 honorReducedMotion 翻回 true（不打属性）时才恢复 OS reduced-motion
// 覆盖。本文件的 reward-chest.js 自身的 prefersReducedMotion() 判定（feed 进
// onChestComplete payload 的 reducedMotion 字段供 QA 断言）未受 013 影响，继续如实反映
// matchMedia 命中结果（见既有用例"prefers-reduced-motion 命中时"），本测试只补 CSS 侧的门。
// ============================================================================================
test('18. reward-chest.css 静态契约：prefers-reduced-motion 兜底块内 .wtj-rc-chest/.wtj-rc-flash 规则均应加 html:not([data-wtj-motion-forced]) 门', function () {
  var RC_CSS_PATH = path.resolve(__dirname, '../../app/web/reward-chest.css');
  var css = readFileSync(RC_CSS_PATH, 'utf8');
  var reducedMotionBlockMatch = css.match(/@media \(prefers-reduced-motion: reduce\)\s*\{([\s\S]*)\}\s*$/);
  assert.ok(reducedMotionBlockMatch, 'reward-chest.css 应包含 prefers-reduced-motion 的兜底块');
  var blockBody = reducedMotionBlockMatch[1];

  ['.wtj-rc-chest', '.wtj-rc-flash'].forEach(function (selector) {
  ...
  var bareMatch = blockBody.match(/(^|[,{};]\s*)\.wtj-rc-(chest|flash)\b/);
