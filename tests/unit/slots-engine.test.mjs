...
//
// WTJ-20260704-083（槽数默认从 5 改为 3）：本文件此前大量用例硬编码"5 个不同发现"来验证
// "填满所有槽 -> onFull -> 溢出不占位"这条状态机行为，隐含假设了 manifest.slots.count 恒为
// 5。真实默认值改为 3 后，这些用例改用 fillWords(sb, realSlotCount(sb))（见下方 WORD_POOL /
// fillWords()）按真实 slot 数量动态取词，不再写死 5——这样无论 manifest 默认值今后是 3 还是
// 任何其它正整数，这些用例验证的都还是"填满所有槽"这个状态机行为本身，而不是巧合地验证"5"
// 这个具体数字。同理，此前写死的 [null, null, null, null, null] 也改用
// emptySlotsArray(sb)（长度 = realSlotCount(sb)）。
//
...
// WTJ-20260704-083：足够大的词池（>= 10，留有余量），供 fillWords() 按 realSlotCount(sb) 动态
// 截取"恰好填满所有槽"所需的那几个不同发现，不再硬编码 5 个。
var WORD_POOL = ['dog', 'cat', 'apple', 'ball', 'star', 'moon', 'sun', 'fish', 'tree', 'frog'];

// 依次 fillSlot('secret-word', ...) n 个来自 WORD_POOL 的不同词，返回每次调用的结果数组
// （与之前手写的 ['dog','cat',...].forEach(...) 等价，只是数量由调用方传入而不是写死）。
function fillWords(sb, n) {
  var results = [];
  var i;
  for (i = 0; i < n; i++) {
    var w = WORD_POOL[i];
    results.push(sb.SLOTS.fillSlot('secret-word', { itemKey: w, renderState: { spriteUrl: w + '.png' } }));
  }
  return results;
}

// 长度 = n、全部为 null 的数组（替代此前写死的 [null, null, null, null, null]）。
function emptySlotsArray(n) {
  var arr = [];
  var i;
  for (i = 0; i < n; i++) {
    arr.push(null);
...
// =====================================================================
test('0. vm 直跑真实 manifest.js：引擎读到的是产品真实数值（slots.count=3, sources=[secret-word, keyboard-milestone]）', function () {
  var sb = createSlotsSandbox();
  // WTJ-20260704-083（开发机验收反馈①）：默认槽数从 5 改为 3，对 3 岁目标用户更合适（5 次
  // 发现才触发一次宝箱奖励，正反馈间隔过长）。
  assert.equal(realSlotCount(sb), 3, '真实 manifest.slots.count 应为 3（WTJ-20260704-083 起默认值）');
...
// =====================================================================
// 2. 发现槽数量可配置（验收 1 / REQ-SLOT-01，WTJ-20260704-083 起默认 3）
// =====================================================================
test('2. 发现槽数量按 manifest.slots.count 配置（验收 1）：getState().slotCount / getSlots().length 均为真实 manifest 的 count', function () {
  var sb = createSlotsSandbox();
  var count = realSlotCount(sb);
  assert.equal(sb.SLOTS.getState().slotCount, count);
  assert.equal(sb.SLOTS.getSlots().length, count);
  console.log('PASS 2: 发现槽数量固定为 ' + count + '。');
});
...
// =====================================================================
test('5. 填满全部槽位 emit onFull 恰好一次（验收 5）：所有槽保持已点亮，继续 fillSlot 新发现返回 filled:false/full:true，不覆盖已有槽', function () {
  var sb = createSlotsSandbox();
  var count = realSlotCount(sb);
  var fullEvents = [];
  sb.SLOTS.onFull(function (snapshot) { fullEvents.push(snapshot); });

  var results = fillWords(sb, count);

  var i;
  for (i = 0; i < count - 1; i++) {
    assert.equal(results[i].filled, true, '第 ' + (i + 1) + ' 次应成功占槽');
    assert.equal(results[i].full, false, '前 ' + (count - 1) + ' 次不应已满');
  }
  assert.equal(results[count - 1].filled, true, '第 ' + count + ' 次（最后一格）应成功占槽');
  assert.equal(results[count - 1].full, true, '第 ' + count + ' 次占满后 full 应为 true');

  assert.equal(fullEvents.length, 1, 'onFull 应恰好被 emit 一次');
  assert.equal(fullEvents[0].full, true);
  assert.equal(fullEvents[0].slots.filter(function (s) { return s !== null; }).length, count);

  // 继续 fillSlot 一个全新发现（词池里紧接着尚未用过的下一个词）：无处可放，不覆盖已有槽，
  // 也不重复 emit onFull。
  var overflowWord = WORD_POOL[count];
  var overflow = sb.SLOTS.fillSlot('secret-word', { itemKey: overflowWord, renderState: { spriteUrl: 'y' } });
  ...
  assert.equal(slotsSnapshot[0].itemKey, WORD_POOL[0], '所有槽保持已点亮的内容，未被 overflow 覆盖');
  assert.equal(sb.hudSetSlotCalls.length, count, 'WTJ_HUD.setSlot 只应被真正占槽的 ' + count + ' 次调用调用，overflow 不再调用');
  console.log('PASS 5: 填满全部 ' + count + ' 槽时 onFull 恰好触发一次，所有槽保持点亮直到 reset()，溢出的新发现不占位、不重复触发。');
});
...
test('5b. 满槽后传入 duplicate 项（非新项）也不重复 emit onFull（P2-4）', function () {
  ...
  // 满槽后再次传入一个"当前几格内已存在"的项（WORD_POOL[0]）：应识别为 duplicate，不占槽、
  // 不重复 emit。
  var dup = sb.SLOTS.fillSlot('secret-word', { itemKey: WORD_POOL[0], renderState: { spriteUrl: WORD_POOL[0] + '.png' } });
  ...
  assert.equal(sb.hudSetSlotCalls.length, count, 'duplicate 项不应再调用 WTJ_HUD.setSlot');
...
// =====================================================================
test('6. reset()：清空所有槽 + 调用 WTJ_HUD.clearSlots() + 防御式通知 WTJ_SECRET.resetRound() / WTJ_KEYBOARD.resetEffectiveKeyCount()；重置后可再次填槽并再次 emit onFull', function () {
  var sb = createSlotsSandbox();
  var count = realSlotCount(sb);
  var fullEvents = [];
  sb.SLOTS.onFull(function (snapshot) { fullEvents.push(snapshot); });
  ...
  assert.deepEqual(Array.from(sb.SLOTS.getSlots()), emptySlotsArray(count), 'reset() 后内部槽应全部清空');
  assert.equal(sb.SLOTS.getState().full, false);

  // 新一轮：之前已经"用过"的 itemKey（WORD_POOL[0]）现在应能重新占槽（因为内部去重表已清空）。
  var r = sb.SLOTS.fillSlot('secret-word', { itemKey: WORD_POOL[0], renderState: { spriteUrl: WORD_POOL[0] + '.png' } });
  assert.equal(r.filled, true);
  assert.equal(r.slotIndex, 0, '新一轮从槽 0 重新开始');
  assert.equal(r.duplicate, false, '清空后的新一轮，同一个词不再被判定为重复');

  // 再次填满所有槽，应再次 emit onFull（第 2 次，不是被"用完"的一次性事件）。
  var i;
  for (i = 1; i < count; i++) {
    var w = WORD_POOL[i];
...
// =====================================================================
test('7. clearSlots()：清空所有槽 + 调 WTJ_HUD.clearSlots()，但不调用 WTJ_SECRET.resetRound() / WTJ_KEYBOARD.resetEffectiveKeyCount()', function () {
  var sb = createSlotsSandbox();
  var count = realSlotCount(sb);
  ...
  assert.deepEqual(Array.from(sb.SLOTS.getSlots()), emptySlotsArray(count));
...
test('10. 混合来源填槽顺序：source 与 itemKey 均不同的多个发现各自占用递增的槽位', function () {
  var sb = createSlotsSandbox();
  var count = realSlotCount(sb);
  // 交替生成 secret-word / keyboard-milestone 两个来源的项，长度 = realSlotCount(sb)（不
  // 再写死 4 项——真实槽数改为 3 后，第 4 项会溢出，不再是"顺序递增占槽"而是"槽已满"）。
  var seq = [];
  var i;
  for (i = 0; i < count; i++) {
    if (i % 2 === 0) {
      seq.push(['secret-word', WORD_POOL[i]]);
    } else {
      seq.push(['keyboard-milestone', 100 * (i + 1)]);
    }
  }
...
test('12. getState()/getSlots() 返回快照：外部修改返回值（含 renderState 字段）不影响引擎内部状态（P2-1）', function () {
  ...
  // onFull 快照同样隔离（011 消费路径）：填满所有槽拿到 onFull 快照，改写它不污染引擎。
  var sb2 = createSlotsSandbox();
  var fullSnap = null;
  sb2.SLOTS.onFull(function (snap) { fullSnap = snap; });
  fillWords(sb2, realSlotCount(sb2));
...
// =====================================================================
// 第 3 部分：WTJ-20260704-066 端到端回归 —— 真实 manifest+slots+keyboard+secretword 四文件
// 按生产加载顺序（manifest → slots → keyboard → secretword）同装一个 sandbox，驱动真实的
// window.WTJ_SLOTS.reset()（五槽满触发宝箱后的真实入口），验证跨轮输入流状态确实被清空。
// =====================================================================
function createFullEngineSandbox() {
  var keydownHandler = null;
  var letterHandlers = [];
  var hudSetSlotCalls = [];

  function makeNode2(tag) {
    var node = {
      tagName: (typeof tag === 'string' ? tag : '').toUpperCase(),
      className: '', src: '', style: {}, children: [], parentNode: null,
      setAttribute: function () {}, addEventListener: function () {},
      appendChild: function (child) { child.parentNode = node; node.children.push(child); return child; },
      removeChild: function (child) {
        var idx = node.children.indexOf(child);
        if (idx >= 0) node.children.splice(idx, 1);
        child.parentNode = null;
  ...
  var fakeWindow = {
    ...
    WTJ_AUDIO: { playWord: function () { return Promise.resolve({ ok: true }); } }
  };

  var body = makeNode2('body');
  var fakeDocument = {
    createElement: function (tag) { return makeNode2(tag); },
    getElementById: function () { return null; },
    body: body
  };

  var sandbox = {
    window: fakeWindow, document: fakeDocument, console: console,
    setTimeout: setTimeout, clearTimeout: clearTimeout, Promise: Promise
  };
  vm.createContext(sandbox);

  // 生产真实加载顺序：manifest → slots（先于 keyboard/secretword）→ keyboard → secretword。
  vm.runInContext(MANIFEST_SRC, sandbox, { filename: 'manifest.js' });
  vm.runInContext(SLOTS_SRC, sandbox, { filename: 'slots.js' });
  vm.runInContext(KEYBOARD_SRC, sandbox, { filename: 'keyboard.js' });
  vm.runInContext(SECRETWORD_SRC, sandbox, { filename: 'secretword.js' });
  ...
  function fireKey(key) { keydownHandler({ key: key, repeat: false }); }
...
test('17. WTJ-20260704-066 端到端：真实键盘输入触发的 WTJ_SLOTS.reset() 必须清空 009 的 rolling buffer —— "do" + reset + "g" 不得跨轮误命中 dog', function () {
  var sb = createFullEngineSandbox();
  var hits = [];
  sb.SECRET.onHit(function (p) { hits.push(p.word); });

  sb.fireKey('d');
  sb.fireKey('o');
  assert.equal(sb.SECRET.getBuffer(), 'do', '真实键盘输入 d/o 后 buffer 应含残留半词 do');

  // 生产真实入口：五槽满触发宝箱后由 010 调用 WTJ_SLOTS.reset()（此处直接调用模拟该触发）。
  sb.SLOTS.reset();
  assert.equal(sb.SECRET.getBuffer(), '', 'WTJ_SLOTS.reset() 必须透传清空 009 的 rolling buffer');

  sb.fireKey('g'); // 新一轮第一个真实按键
  assert.deepEqual(hits, [], 'WTJ_SLOTS.reset() 之后，"do|reset|g" 跨轮拼接不得误命中 dog（端到端真实键盘路径）');
  console.log('PASS 17: 端到端验证 —— 真实键盘输入下 WTJ_SLOTS.reset() 正确透传清空 009 的 rolling buffer，杜绝跨轮误命中 dog。');
});

test('18. WTJ-20260704-066 端到端：真实键盘输入触发的 WTJ_SLOTS.reset() 必须清空 008 的同键节奏门控 —— 新一轮首键不被上一轮"同键>3暂停"吞掉', function () {
  var sb = createFullEngineSandbox();
  var letters = [];
  sb.KEYBOARD.onLetter(function (ch) { letters.push(ch); });

  // 上一轮末尾同键连打触发暂停（真实 manifest pauseAfterCount=3，连打 6 次 'b'）。
  for (var i = 0; i < 6; i++) sb.fireKey('b');
  var countBeforeReset = sb.KEYBOARD.getEffectiveKeyCount();
  assert.equal(countBeforeReset, 3, '连续 6 次同键 b，真实 manifest 阈值 3 下应只计数前 3 次');

  // 生产真实入口：五槽满触发宝箱后由 010 调用 WTJ_SLOTS.reset()。
  sb.SLOTS.reset();
  assert.equal(sb.KEYBOARD.getEffectiveKeyCount(), 0, 'WTJ_SLOTS.reset() 应透传清零 008 的有效键计数');

  sb.fireKey('b'); // 新一轮首键，恰好与上一轮触发暂停的键相同
  assert.equal(sb.KEYBOARD.getEffectiveKeyCount(), 1, 'WTJ_SLOTS.reset() 必须透传清空 008 的同键节奏门控，新一轮首键（即使同键）应被正常计数');
  assert.deepEqual(letters[letters.length - 1], 'B', '新一轮首键应正常触发 onLetter（未被上一轮 streak 吞掉）');
  console.log('PASS 18: 端到端验证 —— 真实键盘输入下 WTJ_SLOTS.reset() 正确透传清空 008 的 lastKeyId/sameKeyStreak，新一轮首键不被上一轮吞掉。');
});
