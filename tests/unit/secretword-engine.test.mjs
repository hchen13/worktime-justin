...
// =====================================================================
test('14. vm 直跑真实 manifest.js：引擎读到的是产品真实词池（含 dog/apple/treasurechest 等 8 词，matchRules 全开）', function () {
  var sb = createSandbox();
  var pool = sb.manifest.secretWords.pool;
  assert.equal(pool.length, 8, '真实 manifest 首批词池应为 8 条');
  ...
  assert.equal(slots.length, 1, 'treasurechest 应命中一次并点亮一槽');
  assert.equal(slots[0].state.spriteUrl, 'assets/sprites/treasure-chest.png',
    'treasurechest 词对应文件名 treasure-chest.png（带连字符），resolveSpritePath 拼成 assets/sprites/treasure-chest.png');
  console.log('PASS 14: 真实 manifest 8 词池 + matchRules 全开；treasurechest → assets/sprites/treasure-chest.png（连字符文件名正确）。');
});
