...
// =====================================================================
// 19. 语言感知词音（WTJ-20260706-011，ZH 秘密词二期脚手架第二片，dormant）
// =====================================================================
...
test('19b. window.WTJ_VOICE_LANG 存在但 ZH 台账为空（isWordZhAvailable 恒 false，当前真实状态）：即便 getEffectiveLanguage()="zh"，playWord 仍收到英文 audioFile', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'apple', spriteFile: 'sprites/apple.png', audioFile: 'audio/words/apple.m4a' }
    ])
  ...
  sb.SECRET.onHit(function () {});
  sb.feed('APPLE');
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.m4a', 'ZH 台账为空时，即便当前生效语言是 zh，也不应切换到未交付的 .zh.m4a 路径');
  console.log('PASS 19b: ZH 台账为空（当前真实状态）→ 即便 effectiveLanguage=zh，仍播英文，NO-SILENT 满足。');
});
...
//     voice-language.js（当前真实、空的 ZH_AVAILABLE_WORD 台账）逐词命中，断言 playWord
//     收到的 audioFile 全部是已交付英文路径、绝无一次 .zh.m4a——直接证明本切片"零用户可见
//     变化"，且从不构造/请求任何尚未交付的中文词音路径（NO-SILENT）。
// =====================================================================
test('20. NO-SILENT 全量扫描：真实 100 词池逐一命中，playWord 收到的 audioFile 全部是已交付英文路径，从未出现 .zh.m4a（当前真实 ZH 台账为空）', function () {
  ...
  assert.equal(sb.window.WTJ_VOICE_LANG.getZhAvailableWordIds().length, 0,
    '前提条件：当前真实 ZH_AVAILABLE_WORD 台账应为空；若此断言失败，说明台账已被改动（008 开始交付），本测试需要同步更新为"部分词应切 ZH"');
  ...
  for (i = 0; i < pool.length; i++) {
    ...
    assert.equal(lastCall.audioFile, entry.audioFile, entry.word + ': playWord 收到的 audioFile 应恰好等于 manifest 里已交付的英文路径 ' + entry.audioFile);
  }
  assert.equal(zhCallCount, 0, '100 词全量扫描中，playWord 不应有任何一次收到 .zh.m4a 路径（当前 ZH 台账为空，NO-SILENT 成立）');
  console.log('PASS 20: 真实 100 词池全量扫描——所有 playWord 调用均为已交付英文路径，零 .zh.m4a 出现，验证 011 切片零用户可见变化 + NO-SILENT。');
});
