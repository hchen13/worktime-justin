...
test('14. vm 直跑真实 manifest.js：引擎读到的是产品真实词池（019 第二批扩展后 100 词：Pack B 99 + 遗留 treasurechest，matchRules 全开）', function () {
  ...
  // 100 = Pack B 词池目标 100（见 manifest.secretWords.poolTargetSize）- 1（WTJ-20260706-011
  // 删除教错 X 发音的 xylophone）+ 1 条非 Pack B 遗留词 treasurechest（见 app/web/manifest.js
  // secretWords 注释）。这个总数不随 DESIGN 后续把个别词从 stub 转 ready 而变化（那只改变某
  // 条目的 spriteStub 标记，不改变 pool 长度），只有 Pack B 目标词表本身变化（新增/移除词）
  // 才需要同步更新本断言。
  assert.equal(pool.length, 100, '019 第二批扩展 + 011 删除 xylophone 后真实词池应为 100 条（Pack B 99 + 遗留 treasurechest 1 条）');
  ...
  console.log('PASS 14: 真实 manifest 100 词池（Pack B 99 + treasurechest，xylophone 已删除）+ matchRules 全开；TREASURECHEST 先命中 treasure 再命中 treasurechest（复合顺序独立触发），后者 → assets/sprites/treasure-chest.png（连字符文件名正确）。');
});
