...
// =====================================================================
// 19. 秘密词完成音恒定英文（WTJ-20260707-011 验收反馈②）
// =====================================================================
// resolveWordAudioEntry() 现恒定返回英文 entry.audioFile，不再查 window.WTJ_VOICE_LANG。
// Ethan 验收反馈②：键盘拼出的秘密词（英文单词）完成发音必须固定英文、与任务叙述语言解耦。
// 因此**无论** WTJ_VOICE_LANG 处于何种状态（缺失 / zh+该词已交付 / en / 调用抛错），playWord
// 收到的都必须是英文 audioFile。下面这组用例逐一钉住"任何 VOICE_LANG 状态都播英文"这一契约
// （历史上这组曾覆盖"语言感知切 .zh.m4a"分支，该分支已随本卡移除）。
...
test('19c. 即便 ZH 台账里该词已交付（isWordZhAvailable 返回 true）且 effectiveLanguage="zh"，秘密词完成音仍固定英文 audioFile（反馈②：与任务叙述语言解耦）', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'apple', spriteFile: 'sprites/apple.png', audioFile: 'audio/words/apple.m4a' }
    ])
  ...
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.m4a', '即便 zh 叙述 + 该词 ZH 已交付，秘密词完成音也固定英文（不再切 .zh.m4a）');
  console.log('PASS 19c: zh 叙述 + 该词 ZH 已交付场景下，秘密词完成音仍固定英文 audioFile——反馈②已落地。');
});
...
// =====================================================================
// 20. 秘密词完成音全池恒英文（WTJ-20260707-011 反馈②）：用真实 100 词 manifest.js + 真实
//     voice-language.js（ZH_AVAILABLE_WORD 台账 100/100 全交付、DEFAULT_MODE='zh'）逐词命中，
//     断言 playWord 收到的 audioFile 对全部 100 词都是**英文**路径，无一切到 .zh.m4a——证明
//     即便在"中文台账全交付 + 默认中文叙述"这个最容易泄漏中文的前提下，秘密词完成音也固定英文。
// =====================================================================
test('20. 全量扫描：真实 100 词池（+ 真实 voice-language.js，100/100 ZH 已交付、默认 zh 叙述）逐一命中，全部 playWord audioFile 均为英文路径，无一 .zh.m4a（反馈②：秘密词恒英文）', function () {
  var sb = createSandbox(); // 真实 manifest.js（100 词）
  // 补挂真实 voice-language.js 到同一沙箱（与 index.html 里 voice-language.js 先于
  // secretword.js 加载的真实顺序一致）。即便它把 DEFAULT_MODE 设为 'zh' 且 ZH 台账 100/100
  // 全交付，resolveWordAudioEntry 现已不再读它、恒返回英文——本测试正是钉住这一点。
  ...
  assert.equal(zhAvailableWordIds.length, 100,
    '前提条件：真实 ZH_AVAILABLE_WORD 台账应为完整 100 条已交付词——本测试要在"最易泄漏中文"的全交付前提下证明秘密词仍恒英文');
  assert.equal(sb.window.WTJ_VOICE_LANG.getEffectiveLanguage(), 'zh',
    "前提条件：默认生效语言应为 'zh'——正是这个设置此前导致秘密词误播中文，现应被解耦");
  ...
  for (i = 0; i < pool.length; i++) {
    ...
    assert.equal(lastCall.audioFile, entry.audioFile, entry.word + ': playWord 收到的 audioFile 应为原样英文路径 ' + entry.audioFile);
    assert.equal(/\.zh\.m4a$/.test(lastCall.audioFile), false, entry.word + ': 秘密词完成音绝不能切到 .zh.m4a（反馈②：恒英文）');
  }
  // 总调用数是 101 而非 100：'treasurechest' 逐字母输入时，缓冲区先在第 8 个字母处完整拼出
  // 子串 'treasure'（同在词池），按 REQ-SEC-10 "复合顺序独立触发"先独立命中一次 'treasure'，
  // buffer 继续增长后再命中 'treasurechest' 本身——既有的、REQ-SEC-10/06 组已覆盖的子串独立
  // 触发行为。两次命中现在都走英文，故全量扫描共 100+1=101 次英文、0 次 .zh.m4a。
  assert.equal(enCallCount, 101, '100 词全量扫描中应有 101 次 playWord 调用收到英文路径：100 条词各自的主命中，' +
    "加上 'treasurechest' 逐字母输入时先独立命中的子串 'treasure' 这 1 次额外命中");
  assert.equal(zhCallCount, 0, '100 词全量扫描中，0 次 playWord 调用切到 .zh.m4a（反馈②：秘密词完成音与任务叙述语言解耦、恒英文）');
  console.log('PASS 20: 真实 100 词池 + 全交付 ZH 台账 + 默认 zh 叙述下全量扫描——全部 101 次命中均播英文，0 次 .zh.m4a，反馈②恒英文契约成立。');
});
