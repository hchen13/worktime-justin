...
// =====================================================================
// 20. NO-SILENT 全池扫描（WTJ-20260706-011 返工）：用真实 100 词 manifest.js + 真实
//     voice-language.js（返工后 ZH_AVAILABLE_WORD 台账 100/100 全交付）逐词命中，断言
//     playWord 收到的 audioFile 对全部 100 个已交付词都是派生的 .zh.m4a 路径，无一原样英文
//     （无 EN fallback 占位）。NO-SILENT：既然全交付，每个词都有真实 .zh.m4a 可播。
// =====================================================================
test('20. NO-SILENT 全量扫描：真实 100 词池逐一命中，全部 100 个已交付词的 playWord audioFile 均切到 .zh.m4a，无一原样英文（返工后 0 缺口，无 EN fallback）', function () {
  ...
  assert.equal(zhAvailableWordIds.length, 100,
    '前提条件：返工后真实 ZH_AVAILABLE_WORD 台账应为完整 100 条已交付词；若此断言失败，说明台账已被改动，本测试需要同步更新');
  var zhAvailableSet = {};
  ...
  // 总调用数是 101 而非 100：'treasurechest'（返工后已交付）逐字母输入时，缓冲区先在第 8 个
  // 字母处完整拼出子串 'treasure'（同为已交付词），按 REQ-SEC-10 "复合顺序独立触发"规则先独立
  // 命中一次 'treasure'（.zh.m4a），buffer 继续增长后再命中 'treasurechest' 本身（entry 循环里
  // 的最后一次命中，同样 .zh.m4a）——这是本引擎既有的、REQ-SEC-10/06 组用例已覆盖的子串独立
  // 触发行为。返工后 treasure 与 treasurechest 均已交付，故这次额外的 'treasure' 命中也是一次
  // .zh.m4a 调用，全量扫描共 100+1=101 次 .zh.m4a、0 次英文回退。
  assert.equal(zhCallCount, 101, '100 词全量扫描中应有 101 次 playWord 调用收到 .zh.m4a 路径：100 条已交付词各自的主命中，' +
    "加上 'treasurechest' 逐字母输入时先独立命中的子串 'treasure'（同为已交付词）这 1 次额外命中");
  assert.equal(enCallCount, 0, '100 词全量扫描中，0 次 playWord 调用是英文路径（返工后全部中文已交付，无 EN fallback 占位）');
  console.log('PASS 20: 真实 100 词池全量扫描——全部 100 个已交付词切 .zh.m4a（含 treasurechest 逐字母命中 treasure 子串的 1 次额外 .zh.m4a），0 次英文回退，NO-SILENT 全池成立。');
});
