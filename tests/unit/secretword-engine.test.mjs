...
var VOICE_LANG_JS_PATH = path.resolve(__dirname, '../../app/web/voice-language.js');
var MANIFEST_SRC = readFileSync(MANIFEST_JS_PATH, 'utf8');
var SECRETWORD_SRC = readFileSync(SECRETWORD_JS_PATH, 'utf8');
var VOICE_LANG_SRC = readFileSync(VOICE_LANG_JS_PATH, 'utf8');
...
// =====================================================================
test('14. vm 直跑真实 manifest.js：引擎读到的是产品真实词池（019 第二批扩展后 100 词：Pack B 99 + 遗留 treasurechest，matchRules 全开）', function () {
  var sb = createSandbox();
  var pool = sb.manifest.secretWords.pool;
  // 100 = 活跃 Pack B 99（WTJ-20260706-011 删除 xylophone，WTJ-20260706-015 用 fox 替换 xray）
  // + 1 条非 Pack B 遗留词 treasurechest（见 app/web/manifest.js
  // secretWords 注释）。这个总数不随 DESIGN 后续把个别词从 stub 转 ready 而变化（那只改变某
  // 条目的 spriteStub 标记，不改变 pool 长度），只有 Pack B 目标词表本身变化（新增/移除词）
  // 才需要同步更新本断言。
  assert.equal(pool.length, 100, '019 第二批扩展 + 011/015 替换 X 词后真实词池应为 100 条（Pack B 99 + 遗留 treasurechest 1 条）');
  ...
  assert.ok(words.indexOf('fox') !== -1 && words.indexOf('xray') === -1 && words.indexOf('xylophone') === -1,
    '真实词池应包含 fox，且不再包含 xray/xylophone');
  ...
  console.log('PASS 14: 真实 manifest 100 词池（Pack B 99 + treasurechest，fox 替代 xray/xylophone）+ matchRules 全开；TREASURECHEST 先命中 treasure 再命中 treasurechest（复合顺序独立触发），后者 → assets/sprites/treasure-chest.png（连字符文件名正确）。');
});
...
// =====================================================================
// 19. 语言感知词音（WTJ-20260706-011，ZH 秘密词二期：85/100 已交付，15 条仍
//     not-delivered，见 app/web/audio/missing-audio.json secretWordsZh[]）
// =====================================================================
// resolveWordAudioEntry() 只在"当前生效语言为 zh 且该词的 ZH 已交付（查
// window.WTJ_VOICE_LANG.isWordZhAvailable()）"两个条件同时成立时才切换到 .zh.m4a 路径；
// 任何一个条件不满足、或 WTJ_VOICE_LANG 缺失/抛错，都必须原样回退英文——这组用例逐条覆盖
// 这些分支，用注入的小词池（而非真实 100 词池，那是下面第 20 组的职责）。
test('19a. 无 window.WTJ_VOICE_LANG（脚本未加载/沙箱缺失）时，playWord 仍收到英文 audioFile（防御式回退，行为不变）', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
  ...
  sb.SECRET.onHit(function () {});
  sb.feed('APPLE');
  assert.equal(sb.audioCalls.length, 1);
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.m4a', '缺 WTJ_VOICE_LANG 时应原样播英文（回退分支）');
  console.log('PASS 19a: window.WTJ_VOICE_LANG 缺失时，语言感知分支安全回退到英文 audioFile。');
});

test('19b. window.WTJ_VOICE_LANG 存在但该词 ZH 仍 not-delivered（isWordZhAvailable 对该词返回 false，例如当前真实仍缺口的 net）：即便 getEffectiveLanguage()="zh"，playWord 仍收到英文 audioFile', function () {
  // WTJ-20260706-011 交付后 apple 已是 ZH 已交付词，不再适合做"未交付"示例——改用 net（当前
  // missing-audio.json secretWordsZh 里仍 status:"not-delivered" 的 15 词之一）。
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'net', spriteFile: 'sprites/net.png', audioFile: 'audio/words/net.m4a' }
    ])
  });
  sb.window.WTJ_VOICE_LANG = {
    getEffectiveLanguage: function () { return 'zh'; },
    isWordZhAvailable: function () { return false; }
  };
  sb.SECRET.onHit(function () {});
  sb.feed('NET');
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/net.m4a', '该词 ZH 未交付时，即便当前生效语言是 zh，也不应切换到未交付的 .zh.m4a 路径');
  console.log('PASS 19b: 该词 ZH 仍 not-delivered（如 net）→ 即便 effectiveLanguage=zh，仍播英文，NO-SILENT 满足。');
});

test('19c. 假想未来场景：ZH 台账里该词已交付（isWordZhAvailable 返回 true）且 effectiveLanguage="zh" → playWord 收到派生的 .zh.m4a 路径，word/spriteFile 不变', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'apple', spriteFile: 'sprites/apple.png', audioFile: 'audio/words/apple.m4a' }
    ])
  ...
  sb.window.WTJ_VOICE_LANG = {
    getEffectiveLanguage: function () { return 'zh'; },
    isWordZhAvailable: function (w) { return w === 'apple'; }
  };
  sb.SECRET.onHit(function () {});
  sb.feed('APPLE');
  assert.equal(sb.audioCalls[0].word, 'apple');
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.zh.m4a', '台账标记该词已交付且当前生效语言为 zh 时，应切到 .zh.m4a 路径');
  console.log('PASS 19c: 假想台账已交付场景下，机制正确切到 .zh.m4a（word/spriteFile 不变，仅 audioFile 切换）——证明 008 交付后无需再改代码。');
});

test('19d. effectiveLanguage="en" 时，即便台账标记该词 ZH 已交付，仍应播英文（分支必须同时满足语言与台账两个条件）', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'apple', spriteFile: 'sprites/apple.png', audioFile: 'audio/words/apple.m4a' }
    ])
  ...
  sb.window.WTJ_VOICE_LANG = {
    getEffectiveLanguage: function () { return 'en'; },
    isWordZhAvailable: function () { return true; }
  };
  sb.SECRET.onHit(function () {});
  sb.feed('APPLE');
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.m4a', 'effectiveLanguage 不是 zh 时，即便该词 ZH 已标记可用，也不应切换语言');
  console.log('PASS 19d: effectiveLanguage=en 时不切 ZH（即便台账标记可用），分支正确同时校验两个条件。');
});

test('19e. window.WTJ_VOICE_LANG.getEffectiveLanguage()/isWordZhAvailable() 抛错时安全回退英文，不影响其它命中反馈', function () {
  var sb = createSandbox({
    manifestObject: injectedManifest([
      { word: 'apple', spriteFile: 'sprites/apple.png', audioFile: 'audio/words/apple.m4a' }
    ])
  ...
  sb.window.WTJ_VOICE_LANG = {
    getEffectiveLanguage: function () { throw new Error('synthetic'); },
    isWordZhAvailable: function () { return true; }
  };
  var hits = [];
  sb.SECRET.onHit(function (p) { hits.push(p.word); });
  assert.doesNotThrow(function () { sb.feed('APPLE'); });
  assert.deepEqual(hits, ['apple'], '语言解析抛错不影响 onHit 事件正常触发');
  assert.equal(sb.audioCalls[0].audioFile, 'audio/words/apple.m4a', '语言解析抛错时应安全回退英文');
  console.log('PASS 19e: WTJ_VOICE_LANG 调用抛错时安全捕获、回退英文，不影响其它命中反馈。');
});

// =====================================================================
// 20. NO-SILENT 全池扫描（WTJ-20260706-011）：用真实 100 词 manifest.js + 真实
//     voice-language.js（当前真实 ZH_AVAILABLE_WORD 台账 85/100 已交付）逐词命中，断言
//     playWord 收到的 audioFile 对 85 个已交付词是派生的 .zh.m4a 路径、对 15 个仍
//     not-delivered 的词是原样英文路径，且从不为 15 个未交付词构造/请求 .zh.m4a（NO-SILENT）。
// =====================================================================
test('20. NO-SILENT 全量扫描：真实 100 词池逐一命中，85 个已交付词的 playWord audioFile 切到 .zh.m4a，15 个仍 not-delivered 的词原样播英文，且这 15 个词从未出现 .zh.m4a', function () {
  var sb = createSandbox(); // 真实 manifest.js（100 词）
  // 补挂真实 voice-language.js 到同一沙箱（与 index.html 里 voice-language.js 先于
  // secretword.js 加载的真实顺序一致；resolveWordAudioEntry 只在命中时才读
  // window.WTJ_VOICE_LANG，不在模块加载时读，故先跑 secretword.js 再补挂不影响结果）。
  vm.runInContext(VOICE_LANG_SRC, sb.sandbox, { filename: 'voice-language.js' });
  assert.ok(sb.window.WTJ_VOICE_LANG, '真实 voice-language.js 应挂载 window.WTJ_VOICE_LANG');
  var zhAvailableWordIds = sb.window.WTJ_VOICE_LANG.getZhAvailableWordIds();
  assert.equal(zhAvailableWordIds.length, 85,
    '前提条件：当前真实 ZH_AVAILABLE_WORD 台账应为 85 条已交付词；若此断言失败，说明台账已被改动，本测试需要同步更新');
  var zhAvailableSet = {};
  zhAvailableWordIds.forEach(function (w) { zhAvailableSet[w] = true; });
  ...
  for (i = 0; i < pool.length; i++) {
    var entry = pool[i];
    var isZhAvailable = !!zhAvailableSet[entry.word];
    var expectedAudioFile = isZhAvailable
      ? entry.audioFile.replace(/\.m4a$/i, '.zh.m4a')
      : entry.audioFile;
    sb.SECRET.resetRound();
    var before = sb.audioCalls.length;
    sb.feed(entry.word.toUpperCase());
    var calls = sb.audioCalls.slice(before);
    assert.ok(calls.length >= 1, entry.word + ' 应至少触发一次 playWord 调用');
    calls.forEach(function (c) {
      if (/\.zh\.m4a$/.test(c.audioFile)) zhCallCount++;
      else enCallCount++;
    });
    var lastCall = calls[calls.length - 1];
    assert.equal(lastCall.word, entry.word, entry.word + ': 最后一次命中（最长优先）应是该词本身');
    assert.equal(lastCall.audioFile, expectedAudioFile, entry.word + ': playWord 收到的 audioFile 应为 ' +
      (isZhAvailable ? 'ZH 已交付后派生的 .zh.m4a 路径 ' : '仍 not-delivered、原样英文路径 ') + expectedAudioFile);
    if (!isZhAvailable) {
      assert.equal(/\.zh\.m4a$/.test(lastCall.audioFile), false, entry.word + ': 仍 not-delivered 的词绝不能被构造出 .zh.m4a 路径（NO-SILENT）');
    }
  }
  // 总调用数是 101 而非 100：'treasurechest'（15 条仍 not-delivered 之一）逐字母输入时，
  // 缓冲区先在第 8 个字母处完整拼出子串 'treasure'（已交付的 85 词之一），按 REQ-SEC-10
  // "复合顺序独立触发"规则先独立命中一次 'treasure'（.zh.m4a），buffer 继续增长后再命中
  // 'treasurechest' 本身（entry 循环里的最后一次命中，按上面逐条断言仍是原样英文路径）——
  // 这是本引擎既有的、REQ-SEC-10/06 组用例已覆盖的子串独立触发行为，并非本测试的新增假设，
  // 只是过去 ZH 台账为空时这次额外的 'treasure' 命中恒定播英文、不可见，现在台账真实交付后
  // 才在这条全量扫描里显形为一次额外的 .zh.m4a 调用。
  assert.equal(zhCallCount, 86, '100 词全量扫描中应有 86 次 playWord 调用收到 .zh.m4a 路径：85 条已交付词各自的主命中，' +
    "加上 'treasurechest' 逐字母输入时先独立命中的子串 'treasure'（同为已交付词）这 1 次额外命中");
  assert.equal(enCallCount, 15, '100 词全量扫描中，恰好 15 次 playWord 调用应仍是英文路径（对应仍 not-delivered 的 15 词各自的主命中）');
  console.log('PASS 20: 真实 100 词池全量扫描——85 个已交付词切 .zh.m4a（含 treasurechest 逐字母命中 treasure 子串的 1 次额外 .zh.m4a）、15 个仍 not-delivered 词原样英文，NO-SILENT 对这 15 词成立。');
});
