...
- App 内拦截 Cmd+Q / Cmd+W / Cmd+H / Cmd+M / Cmd+\`（吞掉/长按识别，不触发退出/关闭/隐藏/最小化/切窗口）；空主菜单去掉默认的快捷键绑定。窗口化调试模式下 Cmd+Q 特意放行（方便开发者关调试窗口），其余几个照样拦截，便于用 `WTJ_WINDOWED=1` 冒烟验证拦截效果而不必进真 kiosk。
- **家长入口（主）**：长按 `Cmd+Q` **≥ 5 秒**（画面底部出现独立进度条），满 5 秒弹出隐藏家长菜单——「退出」（无需口令，直接退出）/「设置…」（每日额度、任务语音语言）/「重置今日使用时长」。未满 5 秒松开无任何动作。
- **家长入口（兜底）**：长按 `Esc` 键 **≥ 5 秒**（独立于上面的进度条，由 `window.wtjEscProgress` 驱动），到时弹出原生对话框要求输入退出口令（`NSSecureTextField`）；口令正确 → 应用退出，口令错误或取消 → 回到全屏内容，需**重新长按满 5 秒**才会再弹出。
- **每日使用时长额度**：默认 30 分钟（家长可调，5~180 分钟范围，持久化到本机 `UserDefaults`），按系统本地日期计算、跨天自动重置。额度用完后进入全屏安静锁屏，普通键盘/鼠标不再触发游戏奖励或声音；家长仍可随时长按 Cmd+Q 打开菜单退出/调额度/重置。
