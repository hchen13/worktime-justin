...
本目录下共 **102 个 PNG**（= 101 个 ready sprite + 1 个共享占位图
...
2. **第二批 93 个 ready sprite + 1 个共享占位图**（卡 WTJ-20260704-006 Pack B 生产词池 →
   本卡 WTJ-20260704-019 第二批集成）——见下方「复制清单（Pack B 词池扩展，WTJ-20260704-019
   第二批集成）」。

101 个 ready sprite = 首批 8 个 v3 baseline（含 7 个与 Pack B 重名、继续沿用 v3 baseline 的词
+ treasure-chest）+ 第二批 93 个 Pack B ready sprite（Pack B **100 个全部 ready** 减去 7 个沿用
v3 baseline 的重名词 = 93 个真正从 Pack B 拷贝的文件）。Pack B 已 100% 产出完毕（卡
WTJ-20260704-054 补齐最后 Z 组，PM 已验收），**不再有 stub 词**；共享占位图
`secret-word-placeholder.png` 现已无 pool 条目引用，作为"未来若有新 stub 词可复用"的备用素材
保留（不被运行时加载，也不计入 101 个 ready sprite）。
...
narrow refresh 的 `2026-07-04 10:58`——`xylophone`/`xray`（卡 WTJ-20260704-052）、
`yoyo`/`yarn`/`yak`（batch-04）、最后 `zebra`/`zipper`/`zucchini`（卡 WTJ-20260704-054，PM
已验收）陆续从 stub 转为 ready，均已落地。**以最新状态为准**：`production_ready_count=100`、
`stubbed_pending_count=0`（Pack B 100 词全部 ready，**不再有 stub 词**）。
...
停留在 `docs/assets/production-pack-b/sprites/` 供参考）。下表列出真正新拷贝进本目录的 93 个
Pack B 文件（85 个首次集成 + 后补的 xylophone/xray/yoyo/yarn/yak/zebra/zipper/zucchini 8 个）
...
| `app/web/assets/sprites/xylophone.png` | `docs/assets/production-pack-b/sprites/xylophone.png` | `7bd8bc736600483570eea4fe12f57967` |
| `app/web/assets/sprites/xray.png` | `docs/assets/production-pack-b/sprites/xray.png` | `1d4a64c55f935b91efbc032345c13060` |
...
**本卡（WTJ-20260704-019 第二批）的裁决**：沿用方案不变——继续把 Pack B 新增的 93 个 sprite
...
101 处词池条目路径写法或改动 `secretword.js`，超出本卡"只动 manifest.js 数据行 + 拷贝素材"的
...
（WTJ-20260704-019）把词池同步扩展到 Pack B 100 词后，本目录当前共 101 个 ready sprite + 1 个
共享占位图（合计 102 个 PNG）。101 个 ready sprite 均用于同一套既有机制：命中反馈 sprite 叠层
...
现已无 pool 条目引用（Pack B 100 词全 ready 后不再有 stub），不被运行时消费，仅作备用素材保留。

## 遗留事项（明确不在本卡处理）

- **统一素材管线**：与 `app/web/assets/PROVENANCE.md` 记录的同一遗留问题——当前是手动
  `cp`/脚本批量复制 + 人工核对 md5，没有构建期自动化同步。Pack B 本批已 100% 产出、全部落地，
  但这套"手动复制 + 更新 `manifest.js` + 追加本文件记录"的流程本身仍未自动化，未来若有新词包
  仍需人工重复，值得后续卡评估建管线。
- **Pack B stub 缺口已清零（历史记录）**：本卡执行期间 DESIGN 分批把 X/Y/Z 组从 stub 补齐为
  ready——`xylophone`/`xray`（卡 WTJ-20260704-052）、`yoyo`/`yarn`/`yak`（batch-04）、最后
  `zebra`/`zipper`/`zucchini`（卡 WTJ-20260704-054，PM 已验收）。现场核对
  `docs/assets/production-pack-b/missing-assets.json`（`updated_at_cst: 2026-07-04 10:58`）：
  `missing_count: 0`，Pack B 100 词**全部 ready，无 stub**。共享占位图
...
- **`basket` / `treasurechest` 词池归属（PM 已裁决保留）**：见 `app/web/MANIFEST.md`
  ...
  `dog / cat / apple / ball / moon / star / car / zoo`。本卡（019 第二批）扩展到 Pack B 100 词后：
  `moon` 已在 Pack B M 组正式补齐为秘密词、`basket` 已确认是 Pack B 100 词正式成员（B 组）。
...
- **性能优化留给 018 卡**：直接使用 1024×1024 原图，不做降采样/裁剪/雪碧图合并，页面通过 CSS
  控制显示尺寸（sprite 叠层约 96–200px，见 `secretword.css`）。本卡新增的 93 个 sprite 同规格，
  同样未做任何降采样/裁剪/合并处理，102 个磁盘 sprite 相对 `performance.maxResidentSprites`
  （20，见 `app/web/manifest.js`）的红线关系留给 018 卡评估（当前秘密词命中是一次一个 sprite
  叠层出现后即移除，非同时常驻 102 张，但 018 卡应确认这条假设在真实播放路径下成立）。
