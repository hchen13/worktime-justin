...
2. **零真实素材**：`audio/words/`、`audio/tasks/`、`audio/sfx/`、`audio/phrases/`
   ...
   `missing-audio.json`。
3. **`loadFileURL` file:// 死穴已由 019 卡在壳层解决，`audio.js` 本身无需改动**：
   本项目运行时最初由 002 卡敲定为 WKWebView `loadFileURL`（`file:` 方案）。在这个运行时下，
   `fetch()`/XHR 对 `file:` 方案资源的读取会被 WebKit 近乎必然地拦截失败——
   `loadFileURL(_:allowingReadAccessTo:)` **只放开标签式子资源**（`<img>` / `<audio>` /
   `<script>` / CSS 引用等）的读取，**并不解除** `fetch`/XHR 对 `file:` 方案的限制（事实
   与"loadFileURL 让同目录访问更宽松"这种直觉相反）。`audio.js` 的加载层（`getBuffer` →
   `loadArrayBuffer`）走的正是 `fetch()`，因此在 file:// 运行时下即使把真实 `.m4a` 放到
   约定路径也不会出声。

   **019 卡的解决方案**：在 `app/shell/main.swift` 里实现了 `WKURLSchemeHandler`
   （`WTJResourceSchemeHandler`），把整个 web 运行时改为经自定义 scheme
   `wtjres://app/index.html` 加载（取代 `loadFileURL` 的 `file:` 方案）。页面与其全部
   相对路径子资源（css/js/img/audio/json）因此统一落在同一个 `wtjres://app/` 源下——
   `fetch()`/XHR 视为**同源**请求，不再触发 CORS 拦截。已用真实 `.m4a` 文件实测验证：
   `fetch('audio/words/<word>.m4a')` 在 `wtjres://` 下返回 `status 200`、
   `response.type === 'basic'`（非 `opaque`，即非跨源降级响应），且能正常
   `arrayBuffer()` 取出完整字节，缺失文件会正确走 reject（不会崩溃或卡死）。

   **`audio.js` 本身不需要任何改动**：`loadArrayBuffer(path)` 一直就是普通同源相对路径
   `fetch(path)`，问题根源始终在"页面加载方式决定的 origin"，不在这个函数的实现方式——
   页面换成 `wtjres://` 加载后，`fetch()` 自动变为同源请求，`getBuffer` → `loadArrayBuffer`
   → `decodeAudioData` → 缓存 → 播放这条链路无需任何代码改动即可在真实素材到位后正常出声。
   （历史遗留：`loadArrayBuffer` 函数上方注释仍写着"019 必须替换本函数"字样——这是 016 卡
   交接时的预判，019 实际执行后发现在壳层解决即可、无需替换该函数；因 019 卡范围明确不改动
   `app/web/` 功能 js 文件，该处旧注释暂未同步更新，见 `app/web/INTEGRATION-STATUS.md`
   第 8 节记录的文档过期项，留待后续小改动清理。）
...
5. **`sfx-manifest.json` 与 `DEFAULT_SFX_MAP`（`audio.js` 内部常量）是两份需要手动
   保持同步的数据**：历史上因 file:// CORS 限制而选择把等价的 key→path 映射内联成
   JS 常量（避免运行时 `fetch()` 读 JSON）；019 用 `wtjres://` scheme 改壳后 fetch JSON
   已同源可用（见 §6.3），但仍保留内联常量以兼容浏览器直开 `file://` 的调试场景，且省一次
   运行时读取。新增/修改 SFX 时两处都要改。
