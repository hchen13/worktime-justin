...
### 返工（WTJ-20260708-004 二轮）：收紧「找到 + 词卡」组合停顿

Ethan 听感拒收第一轮：组合中「找到」和词卡之间仍有明显停顿。根因：运行时/组合的中间空隙 =
find.zh 尾静音（0.12s）+ 词卡首静音（部分词达 0.18s）≈ 0.20-0.30s，读作停顿；另 find.zh 录音
「找—到」两音节间有 ~0.20s 内部静音。修法（-40dB，只裁静音、保两音节各 ≥0.05s pad、不改文案）：
- find.zh：尾留白 0.12→0.05；「找—到」内部静音 0.20→0.10（atrim 两段音节各留 0.05 pad 后拼接，
  找/到 均完整未裁，dur 0.776→0.579）。
- 词卡首留白：88/100 词裁到 ~0.05（词卡尾不动）。
- app 运行时 playComposite(find.zh + 词卡) 与 docs 组合预览（忠实顺序拼接，gen-find-combo-preview.py
