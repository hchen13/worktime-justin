...
- HUD 容器（`.wtj-hud-root` 及其大多数子元素）`pointer-events: none`，仅右侧问号按钮
  ...
  `.wtj-hud-lock`）。
- 明确不做（REQ-SEC-01 / REQ-TASK-01 红线，QA 会盯）：不渲染任何真正的输入框、终端回显条、右侧
  图标列（默认右侧只有问号一个元素）。**WTJ-20260705-019 起的例外**：左下 status lane 新增了
  一个纯装饰的 `.wtj-hud-terminal`（`>_` + activity pip），这不是这条红线的松动——它经 DESIGN
  011（`docs/design/wtj-20260705-011-terminal-prompt-decoration-spec.md`，PM 已验收）与
  WTJ-081 诚实边界判定明确批准，可见内容恒定只有 `>_` 两个静态字符 + 一个不带文本的 pip，
  永不渲染真正的输入框、不回显当前按键/秘密词/用户名/路径/命令输出，见下方「HUD API」一节的
  说明。
