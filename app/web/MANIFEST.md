...
- HUD 容器（`.wtj-hud-root` 及其大多数子元素）`pointer-events: none`，仅右侧问号按钮
  ...
  `.wtj-hud-lock`）。
- 明确不做（REQ-SEC-01 / REQ-TASK-01 红线，QA 会盯）：不渲染任何真正的输入框、右侧图标列
  （默认右侧只有问号一个元素）、不做逐键回显。**WTJ-20260705-019 起的例外**：左下 status lane
  新增了一个纯装饰的 `.wtj-hud-terminal`，最初（DESIGN 011）只允许 `>_` + activity pip 两个
  静态字符，**WTJ-20260705-019b 起 Ethan 明确推翻了这一版本里"永不显示 justin@worktime /
  永不显示秘密词"两条禁令**——现在恒定展示 `justin@worktime:` 前缀，且在秘密词完整拼出的
  完成态事件里短暂显示该词拼写（与对象 sprite 同步出现），详见下方「WTJ-20260705-019b 视觉
  返工」一节。仍然坚持的红线：不建输入框/textarea/contenteditable、不做逐键回显（词展示只在
  "一个词已完整拼出"的完成事件里整体写入，不是实时按键拼接）。
- 素材来源、集成范围与遗留事项见 `app/web/assets/PROVENANCE.md`。

### 全宽 footer 底栏 + 左下角 terminal prompt 装饰条（WTJ-20260705-019）

`hud.js` 的 `mount()` 新增了一个 `.wtj-hud-footer` 全宽容器（`position:fixed`；
`left:0/right:0/bottom:0/width:100%`），把发现槽托盘（`buildTray()`）与 footer 常驻宝箱指示器
（`buildChestIndicator()`）都包在里面——取代此前 086 遗留的、只在 `.wtj-hud-tray-wrap` 内部
局部宽度的 `.wtj-hud-footer-bar` 胶囊（该胶囊仍保留，作为 footer 内部槽位区域更高对比度的
"面板"视觉，两者是同一工作台的两层，不互相替代）。两个子元素相应从各自独立的 `position:fixed`
改为相对这个新父级的 `position:absolute`，数值不变（footer 边缘与视口边缘重合）。

同一批还新增了一个纯展示的 `.wtj-hud-terminal` 装饰条（左下 status lane，贴在 footer 上方），
不通过 `window.WTJ_HUD` 暴露任何读写 API——它没有需要外部驱动的状态机，纯粹是一个静态 + CSS
动画的装饰元素：

- 可见内容恒定是 `>_`（`.wtj-hud-terminal-arrow` 静态文本 `>` + `.wtj-hud-terminal-cursor` 静态
  文本 `_`）+ 一个不带文本的 `.wtj-hud-terminal-pip` 圆点。三者的文本内容在整个运行期内不变，
  没有任何代码路径会往这里追加字符、回显按键或显示秘密词/用户名/路径/命令输出。
- 光标呼吸闪烁（1100ms 一轮，opacity 0.28↔0.52）与窄屏/矮屏的尺寸切换均由 `hud.css` 的纯 CSS
  animation/media query 驱动，不使用 canvas 逐帧重绘（遵守 `app/PERFORMANCE.md` 的
  "禁止每帧 gradient/shadowBlur" 红线）。
- 唯一的动态行为：装饰条订阅了 `window.WTJ_KEYBOARD.onEffectiveKey`（keyboard.js 提供的可选
  依赖，未加载时静默跳过）——该回调只回传一个数字（累计有效键计数），从不携带具体按下的字符。
  每次触发只切换一个纯样式 class（`.is-key-pulse`，140ms 的边框/背景微亮），不触碰 `>_` 的
  文本节点，也不新增/暴露任何 `window.WTJ_HUD` API。

`reward-chest.js`/`reward-chest.css` 的一次性开箱大奖励序列（`.wtj-rc-chest`）同一批从水平居中
的 footer 区域挪到了 footer **右侧**，与 `.wtj-hud-chest-lane` 共用同一组锚点数值（`right:
clamp(16px, 4vw, 32px)`；`bottom: 14px`）——不再在画布正中弹出，避免与中央生成的字母/秘密词
物体抢镜（见 `tests/reports/hud-footer-019/{before,after}` 的截图证据）。

规范来源：`docs/design/wtj-20260705-011-terminal-prompt-decoration-spec.md`（DESIGN 011，
PM 已验收）+ `docs/assets/style/wtj-082/`（footer/chest 视觉）。运行时实现参考了两个未合并的
定向分支（`tl/chest-footer-001`、`tl/terminal-prompt-001b`，两者 base 早于本卡基线，未整分支
合并，只搬运了 hud.js/hud.css/reward-chest.{js,css} 的目标改动）。

### WTJ-20260705-019b 视觉返工（Ethan 截图评审反馈，**推翻上一节部分内容**）

Ethan 目视验收 019 首版截图后给出四条具体返工指示，**其中两条是对上面「全宽 footer 底栏 +
左下角 terminal prompt 装饰条」一节、以及本文件上方「明确不做」红线里"terminal 永不回显"
判定的显式反悔**——不是本卡对早期约束的误读，是产品视觉最终裁决者的直接指令。逐条记录：

1. **header 双语标题居中**：`.wtj-hud-topbar` 的 `justify-content` 从 `space-between` 改为
   `center`；原来单一的 `.wtj-hud-title` 文本节点拆成 `.wtj-hud-title-group` 下的两个子节点——
   `.wtj-hud-title-en`（英文主标题，字体栈换成 `docs/assets/style/visual-style-tile.html`
   参考页 `:root` 声明的 `ui-rounded, "SF Pro Rounded", ...` 圆体优先栈，字重从 081 token 的
   800 提到 900）+ `.wtj-hud-title-zh`（中文副标题，更小号更暗）。锁形 glyph 从 flex 布局摘出
   改为 `position:absolute` 右锚定，避免继续占用 flex 空间把居中的标题挤偏。

2. **footer 全宽底栏，卡槽坐在其上**：083 返工版把 082 doc 建议的高对比度背景/分界线数值
   （`rgba(5,10,18,0.78)` + `rgba(156,180,220,0.16)`）错放到了只有 260~380px 宽的
   `.wtj-hud-footer-bar` 小胶囊上，而真正 `left:0/right:0/width:100%` 的 `.wtj-hud-footer`
   本体反而只有更弱的 0.58 背景，几乎看不出来——效果是三个卡槽读起来像悬浮在一个独立小
   container 里，而不是"坐在"清晰可见的全宽底栏上（问题截图见
   `tests/reports/hud-visual-rework-019b/before/03-chest-active.png`，仓库未跟踪，本地
   保留，见 tests/reports/.gitignore 规则）。本次把 082 那组数值搬到 `.wtj-hud-footer` 本体
   （提亮到 0.82/0.2），`.wtj-hud-tray-wrap--generic`（非五槽底图分支）不再借用
   five-slot-tray.png 专用的正方形留白画布定位技巧，改为贴近 footer 宽度的扁平容器
   （`width: min(720px, 82vw)`），槽径改用与容器尺寸解耦的 `clamp()` 直径。
   `.wtj-hud-footer-bar` 保留但降级为不再抢视觉的过渡层（对比截图见
   `tests/reports/hud-visual-rework-019b/after/03-chest-active.png`）。

3. **terminal prompt 用 `justin@worktime:` 前缀，秘密词完成时显示拼写**（**推翻 DESIGN
   011 与上一节"永不显示 justin@worktime / 永不显示秘密词"两条禁令**）：`.wtj-hud-terminal`
   的可见内容不再是恒定的 `>_`，而是 `.wtj-hud-terminal-prefix`（恒定文本
   `"justin@worktime:"`）+ `.wtj-hud-terminal-word`（idle 为空字符串；秘密词完成态展示区）+
   `.wtj-hud-terminal-cursor`（恒定 `_`）+ activity pip。`hud.js` 新增
   `wireTerminalSecretWordDeferred()`，用 diag.js 同款 `setTimeout(fn, 0)` 宏任务延后手法
   订阅 `window.WTJ_SECRET.onHit`（secretword.js 加载顺序晚于本文件），命中时调用
   `showTerminalSecretWord(word)` 把命中词写入 `.wtj-hud-terminal-word` 并加
   `.is-visible`/`.is-word-pulse`，与 secretword.js 的 sprite 一次性叠层在同一次同步的
   `handleHit()` 调用内出现（天然同步，不需要额外协调时间戳），`TERMINAL_WORD_DISPLAY_MS`
   （1900ms）后自动清空回 idle。**仍然坚持的红线**：不建输入框/textarea/contenteditable、
   不做逐键回显——word 节点只在"一个词已完整拼出"的完成事件里整体写入/清空，不会逐字符
   增量拼接。`hud.js` 新增 `_setClock({setTimeout, clearTimeout})`（与
   task.js/pointer.js/task-templates.js/status-rewards.js 同款可注入时钟模式），供测试
   快进这个和已有的 140ms 按键活跃微亮两个一次性定时器。

4. **字母拖尾更像流星尾迹**：`letter-motion.js` 的 `SPARKLE_PARAMS.sizeFracRange` 从
   `[0.32, 0.8]` 收窄到 `[0.12, 0.32]`（星点明显缩小，新上限恰好等于旧下限），
   `countRange` 从 `[2,4]` 提到 `[3,6]`，新增 `tBiasPower`（`t: Math.pow(Math.random(),
   tBiasPower)`）让星点沿拖尾方向"头密尾疏"，而不是均匀撒在整条拖尾上。`sparkles.js` 本身
   未改动（仍是共享的贴图预渲染 + `drawImage` 绘制层，不引入逐帧 gradient/shadowBlur）。

对应的视觉回归门：`tests/unit/hud.test.mjs` 测试 14~17b（标题组结构/居中、footer 全宽
CSS 契约、terminal 前缀结构、真实 manifest+hud+secretword 集成测试驱动 `d`/`o`/`g` 三个
真实字母验证 terminal 展示与 sprite 同步）、`tests/unit/app-visual-086.test.mjs`（标题字体/
居中的 CSS 静态契约）、`tests/unit/letter-motion.test.mjs`（SPARKLE_PARAMS 数值 + t 分布
统计）。Before/after 截图脚本：`tests/visual/scripts/capture_hud_footer.py`（新增
`05-secret-word-terminal` 状态，用真实 `page.keyboard.type("dog")` 驱动，不是手写 DOM 状态）。
