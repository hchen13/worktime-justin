...
//
// WTJ-20260704-066 缺陷修复（QA 020 对抗评审）：009 resetRound() 与 008
// resetEffectiveKeyCount() 均已补充清空各自的"跨轮输入流状态"——009 的 rolling input buffer
// （字母流匹配用滚动窗口）与 008 的 lastKeyId/sameKeyStreak（连续同键防刷节奏）此前都不随轮次
// 重置，导致跨轮半词误命中（如残留 "do" + 新轮 "g" → 误判 dog）与新一轮首键被上轮"同键>3暂停"
// 状态误吞两个真实缺陷（详见两文件各自「轮次边界」一节）。本文件 reset() 的调用点本身**无需
// 改动**——它已经调用 callSecretResetRound()/callKeyboardResetCount()，两处清理是在 009/008
// 各自现有方法内部补齐的，保持"010 无需感知 009/008 内部具体清了哪些状态"的委托边界不变。
