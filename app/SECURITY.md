...
| 快捷键     | 系统默认行为   | 本 App 行为          |
|-----------|--------------|----------------------|
| Cmd+Q     | 退出 App      | 吞掉（kiosk 模式）；窗口化调试模式特意放行，见下 |
| Cmd+W     | 关闭窗口      | 吞掉                  |
| Cmd+H     | 隐藏 App      | 吞掉                  |
| Cmd+M     | 最小化窗口    | 吞掉                  |
| Cmd+\`    | App 内切换窗口 | 吞掉                  |
...
### 1.2 Esc 不直接退出，长按 + 口令才退出（REQ-EXIT-02 / REQ-EXIT-03）
...
整个文件里，`NSApp.terminate` 只有这一处生产触发路径（`showExitPasswordPrompt` 内、口令
校验通过之后）。窗口化调试模式下菜单绑定的 Cmd+Q 是另一处，但那只在开发者主动带
`--windowed` 启动时存在，不是孩子会接触到的 kiosk 生产路径。

### 1.3 孩子侧没有主动退出入口（REQ-EXIT-04）
...
- **不会、也不能触发应用退出**：web 层和原生退出之间没有任何代码连接。原生侧的
  `WKScriptMessageHandler`（`shell` 消息通道）只做日志打印，没有任何路径把"任务收起"
  的消息接到 `NSApp.terminate` 上。退出的唯一入口就是 1.2 节描述的"家长长按 Esc +
  口令"，跟孩子在界面上做什么完全无关。
...
| REQ-EXIT-03  | 长按 Esc ≥5 秒 + 口令才退出              | `checkEscProgress` / `showExitPasswordPrompt` / `resolveExitPasscode` |
