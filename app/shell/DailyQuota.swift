...
//
//  从 app/shell/main.swift 抽出、不依赖 Cocoa/WebKit/AppDelegate 实例状态的纯函数，专门供
//  tests/swift/main.swift（配 run_daily_quota_reset_test.sh）独立编译 + 运行做单元测试用。
//  app/shell/main.swift 是真正的 App 入口，也是 Swift 顶层语句入口文件（文件末尾直接跑
//  `app.run()`），任何与它一起编译进同一个可执行文件的目标都会在启动时跑到那些顶层语句，
//  没法被安全地"当库 import 后只调函数、不启动 GUI App"——因此把真正需要被独立测试覆盖的
//  纯判定逻辑搬到这个没有任何顶层可执行语句、只含函数声明的文件里：main.swift 与测试二
//  进制分别把它加进各自的 swiftc 编译命令即可复用同一份实现，不会出现"两处日期比较逻辑
//  各写一份、逐渐不同步"的隐患。
//
